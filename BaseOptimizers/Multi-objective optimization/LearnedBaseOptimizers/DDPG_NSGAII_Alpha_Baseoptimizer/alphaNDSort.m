function [FrontNo,MaxFNo] = alphaNDSort(PopObj,PopCon,nSort,alpha )
    [N,M]  = size(PopObj);
    Infeasible           = any(PopCon>0,2);
    PopObj(Infeasible,:) = repmat(max(PopObj,[],1),sum(Infeasible),1) + alpha * repmat(sum(max(0,PopCon(Infeasible,:)),2),1,M);
    if M < 3 || N < 500
