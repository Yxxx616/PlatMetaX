% MATLAB Code
function [offspring] = updateFunc1008(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    cons_sigmoid = 1./(1 + exp(-cons));
    fit_sigmoid = 1./(1 + exp(-popfits));
    
    % Mutation components
    F = 0.5;
    alpha = 0.3;
    diff_term = popdecs(r1,:) - popdecs(r2,:);
    diff_term = diff_term .* (1 - cons_sigmoid(:, ones(1, D)));
    best_term = bsxfun(@minus, x_best, popdecs);
    best_term = best_term .* fit_sigmoid(:, ones(1, D));
    ...
    % Adaptive crossover
    CR = 0.8 * (1 - cons_sigmoid) + 0.2 * fit_sigmoid;
    j_rand = randi(D, NP, 1);
    ...
    reflect_prob = 0.7 * fit_sigmoid + 0.3;
