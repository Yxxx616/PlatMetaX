% MATLAB Code
function [offspring] = updateFunc1027(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    % Elite centroid (unweighted)
    x_elite = mean(popdecs(elite_mask,:), 1);
    ...
    % Adaptive parameters
    F = 0.4 + 0.6 * norm_fits; % Scaling factor [0.4,1.0]
    eta = 0.1 * randn(NP, 1); % Small noise term
