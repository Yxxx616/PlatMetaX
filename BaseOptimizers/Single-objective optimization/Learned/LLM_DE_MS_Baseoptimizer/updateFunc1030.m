% MATLAB Code
function [offspring] = updateFunc1030(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    
    % Normalize fitness to [0,1] with handling for constant fitness case
    f_min = min(popfits);
    f_max = max(popfits);
    if f_max - f_min < 1e-12
        norm_fits = 0.5 * ones(NP, 1);
    else
