% MATLAB Code
function [offspring] = updateFunc1032(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    
    % Normalize fitness to [0,1] range (inverted for minimization)
    f_min = min(popfits);
    f_max = max(popfits);
    if f_max - f_min < 1e-12
        norm_fits = zeros(NP, 1);
    else
        norm_fits = (f_max - popfits) / (f_max - f_min);
    end
    ...
    % Adaptive parameters
    F = 0.6 + 0.2 * norm_fits;       % Scale factor [0.6,0.8]
    ...
    % Dynamic crossover
    CR = 0.9 - 0.4 * norm_fits;      % Crossover rate [0.5,0.9]
    j_rand = randi(D, NP, 1);
