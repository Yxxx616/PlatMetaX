% MATLAB Code
function [offspring] = updateFunc1042(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    F = 0.5 * (1 + norm_cons);  % More violation -> larger step
    ...
    perturbation = norm_fits .* (x_best - popdecs);
    mutants = base_mutants + perturbation;
    
    % Adaptive crossover
    CR = 0.9 - 0.4 * norm_fits;  % Better fitness -> lower CR
    j_rand = randi(D, NP, 1);
    ...
    % For infeasible solutions: reinitialization
