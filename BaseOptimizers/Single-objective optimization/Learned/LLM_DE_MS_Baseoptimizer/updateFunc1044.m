% MATLAB Code
function [offspring] = updateFunc1044(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    direction = zeros(NP, D);
    for i = 1:NP
        weighted_diff = zeros(1, D);
        for j = 1:elite_num
            weighted_diff = weighted_diff + w(j)*(elite_pool(j,:) - popdecs(i,:));
        end
        norm_diff = norm(weighted_diff) + eps;
        direction(i,:) = weighted_diff / norm_diff;
    end
