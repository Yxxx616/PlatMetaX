% MATLAB Code
function [offspring] = updateFunc1071(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    
    % 1. Normalize fitness and constraints
    f_min = min(popfits);
    f_max = max(popfits);
    norm_fits = (popfits - f_min) / (f_max - f_min + eps);
    
    c_max = max(abs(cons)) + eps;
    norm_cons = abs(cons) / c_max;
    
    % 2. Calculate selection weights
    weights = (1 - norm_fits) ./ (1 + norm_cons);
    weights = weights / sum(weights);
    
    % 3. Compute weighted centroid
    x_cent = weights' * popdecs;
    
    % 4. Identify best individual (considering constraints)
    [~, best_idx] = min(popfits + 1000*norm_cons);
    x_best = popdecs(best_idx, :);
    
    % 5. Create direction vectors
    delta_best = x_best(ones(NP,1), :) - popdecs;
    delta_cent = x_cent(ones(NP,1), :) - popdecs;
    
    % 6. Random differential vectors
    idx = randperm(NP);
    r1 = idx(1:NP);
    r2 = idx(mod(1:NP, NP)+1);
    diff_vec = popdecs(r1,:) - popdecs(r2,:);
    
    % 7. Adaptive scaling factors
    F_best = 0.4 + 0.3 * norm_cons;
    F_cent = 0.3 * (1 - norm_fits);
    
    % 8. Generate mutant vectors
    mutants = popdecs + ...
              F_best(:, ones(1,D)) .* delta_best + ...
              F_cent(:, ones(1,D)) .* delta_cent + ...
              0.1 * diff_vec;
    
    % 9. Constraint-aware crossover
    CR = 0.9 * (1 - norm_cons.^2);
    j_rand = randi(D, NP, 1);
    mask = rand(NP,D) < CR(:, ones(1,D));
    mask(sub2ind([NP,D], (1:NP)', j_rand)) = true;
    
    % 10. Create offspring
    offspring = popdecs;
    offspring(mask) = mutants(mask);
    
    % 11. Boundary handling with reflection
    lb_rep = repmat(lb, NP, 1);
    ub_rep = repmat(ub, NP, 1);
    below = offspring < lb_rep;
    above = offspring > ub_rep;
    offspring(below) = 2*lb_rep(below) - offspring(below);
    offspring(above) = 2*ub_rep(above) - offspring(above);
    
    % 12. Final bounds check
    offspring = min(max(offspring, lb_rep), ub_rep);
end