% MATLAB Code
function [offspring] = updateFunc1072(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    % 3. Compute weighted centroid and best individual
    ...
    delta_rand = popdecs(r1,:) - popdecs(r2,:);
    
    mutants = popdecs + ...
              F1(:, ones(1,D)) .* delta_best + ...
              F2(:, ones(1,D)) .* delta_cent + ...
              F3(:, ones(1,D)) .* delta_rand;
