% MATLAB Code
function [offspring] = updateFunc1076(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    % 2. Calculate adaptive weights and centroid
    weights = (1 - norm_fits) .* (1 - norm_cons);
    weights = weights / sum(weights);
    x_cent = weights' * popdecs;
    
    % 3. Identify best and elite solutions
    ...
    F2 = 0.5 - 0.3 * norm_cons;
    F3 = 0.4 * (1 - norm_fits .* norm_cons);
