% MATLAB Code
function [offspring] = updateFunc108(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    ...
    % 4. Create opposition point
    x_opp = lb + ub - x_base;
    
    % 5. Generate mutation vectors (vectorized)
    ...
    v = repmat(x_base, NP, 1) + F1.*diff1 + F2.*diff2;
    
    % 6. Adaptive crossover rate
