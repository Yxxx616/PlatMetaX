% MATLAB Code
function [offspring] = updateFunc1080(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    % 9. Special boundary handling for constrained solutions
    ...
    feasible = ~cons_rep;
    offspring(feasible,:) = min(max(offspring(feasible,:), lb_rep(feasible,:)), ub_rep(feasible,:));
