% MATLAB Code
function [offspring] = updateFunc1085(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    F = 0.4 + 0.4 * rand(NP, 1);  % Dynamic F
    F_mat = F(:, ones(1,D));
    CR = min(max(0.7 + 0.1*randn(NP,1), 0.1), 0.9);  % Normally distributed CR
    CR_mat = CR(:, ones(1,D));
    ...
    % 6. Crossover with normal distribution CR
    j_rand = randi(D, NP, 1);
    ...
    % Final bounds check and clamping
