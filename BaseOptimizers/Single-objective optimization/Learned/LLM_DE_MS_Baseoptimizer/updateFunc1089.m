% MATLAB Code
function [offspring] = updateFunc1089(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    mutants = popdecs + F_e.*diff_elite + F_r.*diff_rand + F_c.*diff_cons;
    ...
    % 9. Boundary handling with repair
    ...
    offspring(below) = (popdecs(below) + lb_rep(below))/2 + ...
        r(below).*(popdecs(below) - lb_rep(below));
    offspring(above) = (popdecs(above) + ub_rep(above))/2 - ...
        r(above).*(ub_rep(above) - popdecs(above));
