% MATLAB Code
function [offspring] = updateFunc111(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    ...
    x_best = mean(elites, 1);  % Center of elite individuals
    
    % Opposition point of x_best
    x_opp = lb + ub - x_best;
    ...
    F = 0.3 + 0.5 * (1 - norm_fits) + 0.2 * norm_cons;
    F = repmat(F, 1, D);
