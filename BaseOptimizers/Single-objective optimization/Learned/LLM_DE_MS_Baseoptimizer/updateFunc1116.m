% MATLAB Code
function [offspring] = updateFunc1116(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    f_max = max(popfits);
    c_max = max(abs(cons));
    w = 1 - 0.7*(popfits-f_min)/(f_max-f_min+eps) - 0.3*abs(cons)/(c_max+eps);
    w = w(:);
    ...
    % 6. Dynamic crossover
    CR = 0.8 + 0.2*(popfits-f_min)/(f_max-f_min+eps);
