% MATLAB Code
function [offspring] = updateFunc1118(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    f_max = max(popfits);
    c_max = max(abs(cons));
    w = 0.7*(1 - (popfits-f_min)/(f_max-f_min+eps)) + ...
        0.3*(1 - abs(cons)/(c_max+eps));
    w = w(:);
    ...
    % 6. Dynamic crossover
    CR = 0.9 - 0.2*(popfits-f_min)/(f_max-f_min+eps);
