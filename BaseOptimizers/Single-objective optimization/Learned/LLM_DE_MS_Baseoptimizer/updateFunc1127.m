% MATLAB Code
function [offspring] = updateFunc1127(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    F = 0.5 + 0.3 * (1 - abs(norm_f));
    CR = 0.7 + 0.2 * r_c * (1 - c ./ (max(c) + eps));
    ...
    r1 = randi(NP, NP, 1);
    r2 = randi(NP, NP, 1);
    mutants = x_best + F.*(d_elite + d_div) + F.*(popdecs(r1,:) - popdecs(r2,:));
