% MATLAB Code
function [offspring] = updateFunc1128(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    F = 0.4 + 0.4 * tanh(norm_f); % Adaptive scaling factor
    ...
    reflect_prob = 0.3 + 0.4 * r_c; % More reflection for constrained problems
    reflect = rand(NP,D) < reflect_prob;
    offspring(out_of_bounds & reflect) = 2*lb(out_of_bounds & reflect) - offspring(out_of_bounds & reflect);
    offspring(out_of_bounds & ~reflect) = 2*ub(out_of_bounds & ~reflect) - offspring(out_of_bounds & ~reflect);
