% MATLAB Code
function [offspring] = updateFunc1130(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    F = 0.5 + 0.3 * tanh(norm_f);
    CR = 0.9 - 0.5 * (c ./ (max(c) + eps));
    
    % 3. Direction vectors
    % Elite direction
    elite_mask = popfits < f_mean;
    if any(elite_mask)
        elite_w = exp(-norm_f(elite_mask).^2);
    ...
    r1 = randi(NP, NP, 1);
    r2 = randi(NP, NP, 1);
    mutants = x_best + F.*(d_elite + (1-r_c)*d_div + r_c*d_cons) + F.*(popdecs(r1,:) - popdecs(r2,:));
