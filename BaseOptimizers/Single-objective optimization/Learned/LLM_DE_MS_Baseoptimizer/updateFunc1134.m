% MATLAB Code
function [offspring] = updateFunc1134(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    eps_val = 1e-10;
    ...
    % 3. Adaptive scaling
    alpha = 1 ./ (1 + exp(-5*(c_mean-0.5)));
    d_composite = alpha*d_fit + (1-alpha)*(0.7*d_cons + 0.3*d_div);
    
    % 4. Mutation
    F = 0.5 * (1 + tanh(popfits - f_mean));
