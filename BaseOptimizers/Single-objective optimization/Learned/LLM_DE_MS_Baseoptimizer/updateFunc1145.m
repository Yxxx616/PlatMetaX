% MATLAB Code
function [offspring] = updateFunc1145(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    eps_val = 1e-10;
    ...
    % 3. Adaptive weights
    alpha = 0.5*(1 + tanh(10*(rho - 0.5)));
    beta = 0.5*(1 - rho);
    d_comp = alpha*d_fit + (1-alpha).*(beta*d_feas + (1-beta)*d_div);
