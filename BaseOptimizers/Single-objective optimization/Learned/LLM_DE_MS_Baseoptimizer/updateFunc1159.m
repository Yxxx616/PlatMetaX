% MATLAB Code
function [offspring] = updateFunc1159(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    % 7. Enhanced mutation with dual guidance
    mutants = popdecs + ...
        F1 * (x_elite - popdecs) + ...
        F2 * (x_feas - popdecs) .* repmat(w_feas, 1, D) + ...
        F3 * (popdecs(r1,:) - popdecs(r2,:)) .* repmat(1-w_feas, 1, D);
