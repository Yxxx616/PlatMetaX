% MATLAB Code
function [offspring] = updateFunc1169(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    % 6. Adaptive scaling factors (modified from previous version)
    F_elite = 0.8 + 0.2*rho;
    F_feas = 0.6*(1-rho);
    F_div = 0.4*rho;
    
    % 7. Triple-component mutation (modified coefficients)
    mutants = popdecs + ...
        F_elite * (x_elite - popdecs) + ...
        F_feas * (x_feas - popdecs) .* w_feas + ...
        F_div * (popdecs(r1,:) - popdecs(r2,:)) .* (1-w_feas);
    
    % 8. Adaptive crossover with higher probability
    CR = 0.95 - 0.4*rho;
    mask = rand(NP, D) < CR;
    j_rand = randi(D, NP, 1);
