% MATLAB Code
function [offspring] = updateFunc1170(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    w_feas = 1./(1 + exp(-5*(1 - abs_cons./max_cons)));
    ...
    offspring(lb_mask) = lb(lb_mask) + 0.5*rand(sum(lb_mask(:)),1).*(ub(lb_mask)-lb(lb_mask));
    offspring(ub_mask) = lb(ub_mask) + 0.5*rand(sum(ub_mask(:)),1).*(ub(ub_mask)-lb(ub_mask));
