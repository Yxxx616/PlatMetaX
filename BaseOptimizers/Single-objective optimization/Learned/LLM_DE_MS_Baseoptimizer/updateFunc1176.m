% MATLAB Code
function [offspring] = updateFunc1176(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    
    % 1. Identify elite solutions (top 30% this time)
    [~, sorted_idx] = sort(popfits, 'descend');
    elite_size = max(1, floor(0.3*NP));
    elites = popdecs(sorted_idx(1:elite_size), :);
    
    % 2. Enhanced feasibility calculation
    ...
    % 3. Generate unique random indices with better diversity
    r1 = randi(NP, NP, 1);
    r2 = randi(NP, NP, 1);
    ...
    % 5. Sharper feasibility weights
    abs_cons = abs(cons);
    max_cons = max(abs_cons) + 1e-10;
    w_feas = 1./(1 + exp(5*(abs_cons./max_cons - 0.7)));
    ...
    % 7. Enhanced four-component mutation
    sigma_cons = abs_cons./max_cons;
    mutants = popdecs + ...
        F_elite * (x_elite - popdecs) + ...
        F_feas * (x_feas - popdecs) .* w_feas + ...
        F_div * (popdecs(r1,:) - popdecs(r2,:)) .* (1-w_feas) + ...
        F_cons * randn(NP,D) .* sigma_cons;
    
    % 8. Dynamic crossover probability with better exploration
    CR = 0.9 - 0.4*rho;
    mask = rand(NP, D) < CR;
    j_rand = randi(D, NP, 1);
    ...
    % Final bounds enforcement with clipping
