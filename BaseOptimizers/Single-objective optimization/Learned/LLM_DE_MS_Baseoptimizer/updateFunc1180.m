% MATLAB Code
function [offspring] = updateFunc1180(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    mutants = popdecs + ...
        ...
        0.25 * (1-w_feas) .* randn(NP,D);
    
    % 8. Adaptive crossover with feasibility-based CR
    CR = 0.9 - 0.4*(1-w_feas);
    mask = rand(NP, D) < CR;
    j_rand = randi(D, NP, 1);
    ...
    % 9. Boundary handling with bounce-back
