% MATLAB Code
function [offspring] = updateFunc1194(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    % Diversity weights (distance to centroid)
    ...
    F1 = 0.7 + 0.2 * randn(NP, 1);
    F2 = 0.6 + 0.3 * randn(NP, 1);
    F3 = 0.5 + 0.4 * randn(NP, 1);
