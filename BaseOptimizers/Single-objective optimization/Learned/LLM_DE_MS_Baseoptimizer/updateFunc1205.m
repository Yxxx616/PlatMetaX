% MATLAB Code
function [offspring] = updateFunc1205(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    f_min = min(popfits);
    f_max = max(popfits);
    F_f = 0.5 + 0.5 * (popfits - f_min) ./ (f_max - f_min + eps);
    
    c_max = max(abs(cons));
    F_c = 0.5 * (1 + exp(-abs(cons) ./ (c_max + eps)));
    F = F_f .* F_c;
    ...
    term1 = F .* (x_best - popdecs);
    term2 = (1 - F) .* (x_feas - x_inf);
    term3 = 0.5 * (popdecs(r1,:) - popdecs(r2,:));
