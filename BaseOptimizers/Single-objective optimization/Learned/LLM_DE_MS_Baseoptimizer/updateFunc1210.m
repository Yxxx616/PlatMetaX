% MATLAB Code
function [offspring] = updateFunc1210(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    % 5. Generate multiple difference vectors
    idx = (1:NP)';
    ...
    diff_avg = (diff1 + diff2 + diff3 + diff4)/4;
    
    % 6. Mutation
    mutants = popdecs + bsxfun(@times, F, direction) + 0.5*diff_avg;
