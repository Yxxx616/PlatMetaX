% MATLAB Code
function [offspring] = updateFunc1212(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    cv_abs = abs(cons);
    max_cv = max(cv_abs);
    w = 1./(1 + exp(-10*cv_abs/(max_cv + eps)));
    ...
    diff_avg = (diff1 + diff2)/2 + 0.5*diff3;
    
    % 6. Mutation
    mutants = popdecs + bsxfun(@times, F, direction) + 0.8*diff_avg;
