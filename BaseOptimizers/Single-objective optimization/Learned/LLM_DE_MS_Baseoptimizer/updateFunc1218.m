% MATLAB Code
function [offspring] = updateFunc1218(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    eps_val = 1e-10;
    ...
    % 3. Hybrid mutation with fitness-direction guidance
    idx = (1:NP)';
    ...
    dir_factor = popfits(r3)./(abs(popfits(r3)) + eps_val);
    mutants = x_wbest(ones(NP,1), :) + F .* diff1 + ...
              0.5 * F .* diff2 .* dir_factor(:, ones(1, D));
    ...
    CR_min = 0.3;
    CR_max = 0.9;
    CR = CR_min + (CR_max - CR_min) * (1 - ranks/NP) .* (1 - cv_abs/(max_cv + eps_val));
    CR = CR(:, ones(1, D));
