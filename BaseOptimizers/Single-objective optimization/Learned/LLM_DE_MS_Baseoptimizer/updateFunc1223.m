% MATLAB Code
function [offspring] = updateFunc1223(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    eps_val = 1e-10;
    ...
    F = 0.4 + 0.4 * (1 - cv_abs./(max_cv + eps_val)) .* ...
    ...
    ranks = zeros(NP, 1);
    ranks(rank_idx) = 1:NP;
    CR = 0.85 * (1 - ranks/NP) .* (1 - cv_abs/(max_cv + eps_val));
    CR = CR(:, ones(1, D));
