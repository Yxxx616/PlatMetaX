% MATLAB Code
function [offspring] = updateFunc1230(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    eps_val = 1e-10;
    ...
    % 4. Adaptive weights based on constraint violation
    cv_abs = abs(cons);
    max_cv = max(cv_abs);
    beta1 = 0.6 * (1 - cv_abs/(max_cv + eps_val));
    beta2 = 0.3 * rand(NP,1);
    beta3 = 0.1 * rand(NP,1);
    ...
    % 6. Mutation with Gaussian perturbation
    sigma = 0.05 * (ub - lb);
