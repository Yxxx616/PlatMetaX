% MATLAB Code
function [offspring] = updateFunc1231(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    % 2. Generate random indices matrix (avoid self)
    idx = 1:NP;
    R = zeros(NP, 4);
    for i = 1:NP
    ...
    T = max(abs(popfits)) / 10; % Temperature
    exp_f = exp(-popfits/T);
    w = exp_f(R(:,1:2)) ./ (exp_f(R(:,1:2)) + exp_f(R(:,3:4)));
    ...
    % 5. Adaptive scaling factor based on constraints
    mean_c = mean(cons);
    std_c = std(cons) + 1e-10;
    F = 0.5 * (1 + tanh((cons - mean_c)/std_c));
    F = F(:, ones(1, D));
    
    % 6. Balance between elite and difference directions
    mean_f = mean(popfits);
    std_f = std(popfits) + 1e-10;
