% MATLAB Code
function [offspring] = updateFunc1236(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    % 5. Rank-based adaptive balance
    [~, rank_idx] = sort(popfits);
    ranks = zeros(NP, 1);
    ranks(rank_idx) = 1:NP;
    alpha = 0.2 + 0.6 * (ranks / NP);
    alpha = alpha(:, ones(1, D));
    
    % 6. Dynamic scaling factors
    F = 0.4 + 0.5 * (1 - ranks/NP);
    F = F(:, ones(1, D));
    
    % 7. Constraint-aware perturbation
    beta = 0.1 * tanh(abs(cons));
    epsilon = 0.01 * randn(NP, D);
    cons_perturb = beta .* sign(cons) .* epsilon;
    
    % 8. Mutation with adaptive components
