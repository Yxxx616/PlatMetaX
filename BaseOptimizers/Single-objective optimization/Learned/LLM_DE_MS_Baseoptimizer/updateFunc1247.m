% MATLAB Code
function [offspring] = updateFunc1247(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    d_div = sign(cons) .* (popdecs(R(:,4),:) - popdecs(R(:,2),:));
    
    % 5. Compute adaptive scaling factors
    F = 0.4 + 0.3 ./ (1 + exp(-abs(cons))) .* randn(NP, 1);
    F = F(:, ones(1, D));
    
    % 6. Mutation with weighted directions
    mutants = popdecs + F .* (0.7*d_elite + 0.2*d_fit + 0.1*d_div);
    
    % 7. Rank-based adaptive crossover
    [~, rank_idx] = sort(popfits);
    ranks = zeros(NP, 1);
    ranks(rank_idx) = 1:NP;
    CR = 0.95 - 0.65 * (ranks/NP);
    CR = CR(:, ones(1, D));
