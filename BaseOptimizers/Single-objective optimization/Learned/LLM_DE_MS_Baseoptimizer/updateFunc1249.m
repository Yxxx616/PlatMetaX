% MATLAB Code
function [offspring] = updateFunc1249(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    d_fit = weights(1)*(popdecs(top3(1),:) - popdecs(top3(2),:)) + ...
            weights(2)*(popdecs(top3(1),:) - popdecs(top3(3),:));
    d_cons = sign(cons) .* (popdecs(R(:,4),:) - popdecs(R(:,2),:));
    
    % 5. Compute adaptive scaling factors
    F_base = 0.5 + 0.3 * tanh(abs(cons));
    F = F_base .* (1 + 0.1*randn(NP,1));
    F = min(max(F, 0.1), 1.0);
    F = F(:, ones(1, D));
    
    % 6. Mutation with weighted directions
    mutants = popdecs + F .* (0.7*d_elite + 0.2*d_fit(ones(NP,1),:) + 0.1*d_cons);
