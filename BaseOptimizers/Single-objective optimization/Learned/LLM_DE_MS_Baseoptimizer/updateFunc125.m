% MATLAB Code
function [offspring] = updateFunc125(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    ...
    F = 0.3 + 0.5 * (1 - sqrt(norm_fits .* norm_cons));
    F = repmat(F, 1, D);
    ...
    v = repmat(x_best, NP, 1) + F .* diff1 + 0.7*F .* diff2;
    ...
    % Quality-based reflection with adaptive range
    reflect_factor = 0.1 + 0.8 * repmat(weights, 1, D);
    rand_vals = rand(NP, D);
    
    offspring(below_lb) = lb_rep(below_lb) + reflect_factor(below_lb) .* ...
                         rand_vals(below_lb) .* (ub_rep(below_lb) - lb_rep(below_lb));
    offspring(above_ub) = ub_rep(above_ub) - reflect_factor(above_ub) .* ...
                         rand_vals(above_ub) .* (ub_rep(above_ub) - lb_rep(above_ub));
