% MATLAB Code
function [offspring] = updateFunc1259(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    % 6. Mutation with weighted directions (70% elite, 20% fitness, 10% random)
    mutants = popdecs + F .* (0.7*d_elite + 0.2*d_fit + 0.1*d_rand);
    
    % 7. Rank-based adaptive crossover
    [~, rank_idx] = sort(popfits);
    ranks = zeros(NP, 1);
    ranks(rank_idx) = 1:NP;
    CR = 0.9 - 0.5 * (ranks/NP).^2;
    CR = CR(:, ones(1, D));
