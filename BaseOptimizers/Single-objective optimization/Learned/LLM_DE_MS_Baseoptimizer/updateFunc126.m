% MATLAB Code
function [offspring] = updateFunc126(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    ...
    F = 0.2 + 0.6 * (1 - sqrt(norm_fits .* norm_cons));
    F = repmat(F, 1, D);
    ...
    v = repmat(x_best, NP, 1) + F .* diff1 + 0.8*F .* diff2;
    
    % Dynamic crossover rates with constraint awareness
    CR = 0.85 - 0.45 * norm_cons;
    CR = repmat(CR, 1, D);
    ...
    reflect_factor = 0.2 + 0.7 * repmat(weights, 1, D);
    rand_vals = rand(NP, D);
