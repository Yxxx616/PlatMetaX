% MATLAB Code
function [offspring] = updateFunc1262(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    d_cons = bsxfun(@times, popdecs(R(:,1),:) - popdecs(R(:,2),:), cons_factor(:, ones(1, D)));
    
    % 5. Adaptive scaling factors
    f_mean = mean(popfits);
    f_std = std(popfits) + eps;
    norm_fits = (popfits - f_mean) / f_std;
    F = 0.4 + 0.3 ./ (1 + exp(-5 * norm_fits));
    F = F(:, ones(1, D));
