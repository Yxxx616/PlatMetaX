% MATLAB Code
function [offspring] = updateFunc1264(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    F = 0.4 + 0.3 ./ (1 + exp(-norm_fits));
    F = F(:, ones(1, D));
    
    % 6. Mutation with weighted directions (adjusted weights)
