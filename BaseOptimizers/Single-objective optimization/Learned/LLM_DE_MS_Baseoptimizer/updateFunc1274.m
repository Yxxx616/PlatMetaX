% MATLAB Code
function [offspring] = updateFunc1274(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    d_cons = bsxfun(@times, popdecs(R(:,1),:) - popdecs(R(:,2),:), cons_factor);
    ...
    % 5. Combined mutation with adjusted weights
    mutants = popdecs + F .* (0.6*d_elite + 0.3*d_fit + 0.1*d_cons);
    
    % 6. Rank-based crossover with higher base CR
    CR = 0.95 - 0.5 * (ranks/NP);
    CR = CR(:, ones(1, D));
    mask = rand(NP, D) < CR;
    j_rand = randi(D, NP, 1);
