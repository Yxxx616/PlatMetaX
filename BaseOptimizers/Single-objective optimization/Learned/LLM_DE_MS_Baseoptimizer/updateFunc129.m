% MATLAB Code
function [offspring] = updateFunc129(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    ...
    w_fit = 0.6 * (1 - norm_fits);
    w_con = 0.4 * (1 - norm_cons);
    w_rand = 1 - sqrt(w_fit .* w_con);
    
    % Create mutation vectors
    diff_fit = popdecs(r1,:) - popdecs(r2,:);
    diff_con = popdecs(r3,:) - popdecs(r4,:);
    v = repmat(x_elite, NP, 1) + ...
        repmat(w_fit, 1, D) .* diff_fit + ...
        repmat(w_con, 1, D) .* diff_con + ...
        repmat(w_rand, 1, D) .* (rand(NP,D)-0.5);
    
    % Adaptive crossover rates
    CR = 0.85 * (1 - norm_cons) + 0.1;
    CR = repmat(CR, 1, D);
    ...
    reflect_factor = 0.3 + 0.5 * repmat(1 - norm_fits, 1, D) .* rand(NP, D);
