% MATLAB Code
function [offspring] = updateFunc1297(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    if max_cons > min_cons
        F = 0.5 + 0.5 * (cons - min_cons) / (max_cons - min_cons);
    else
        F = 0.5 * ones(NP, 1);
    end
    ...
    max_fit = max(popfits);
    if max_fit > min_fit
        weights = 0.8 * (popfits - min_fit) / (max_fit - min_fit) + 0.2;
    else
        weights = 0.5 * ones(NP, 1);
    end
