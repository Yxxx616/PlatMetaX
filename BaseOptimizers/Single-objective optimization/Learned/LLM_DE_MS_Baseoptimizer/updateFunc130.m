% MATLAB Code
function [offspring] = updateFunc130(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    ...
    elite_weights = (f_max - popfits(elite_idx)) / sum(f_max - popfits(elite_idx));
    ...
    beta = 0.5 * (1 - norm_fits);
    gamma = 0.1 * (1 - alpha .* beta);
    
    % Create mutation vectors
    diff_fit = popdecs(r1,:) - popdecs(r2,:);
    diff_con = popdecs(r3,:) - popdecs(r4,:);
    v = repmat(x_elite, NP, 1) + ...
        repmat(alpha, 1, D) .* diff_fit + ...
        repmat(beta, 1, D) .* diff_con + ...
        repmat(gamma, 1, D) .* randn(NP,D);
    
    % Adaptive crossover rates
    CR = 0.8 * (1 - norm_cons) + 0.1;
    CR = repmat(CR, 1, D);
