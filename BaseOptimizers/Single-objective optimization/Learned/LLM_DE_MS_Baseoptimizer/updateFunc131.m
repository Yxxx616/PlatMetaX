% MATLAB Code
function [offspring] = updateFunc131(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    ...
    elite_weights = exp(-norm_fits(elite_idx)) / sum(exp(-norm_fits(elite_idx)));
    ...
    % Calculate weights
    w1 = 0.4 + 0.3 * (1 - norm_fits);
    w2 = 0.5 * norm_fits .* (1 - norm_cons);
    w3 = 0.3 * norm_cons;
    w4 = 0.1 * (1 - norm_fits) .* norm_cons;
    
    % Create mutation vectors
    diff_fit = popdecs(r1,:) - popdecs(r2,:);
    diff_con = popdecs(r3,:) - popdecs(r4,:);
    v = repmat(x_elite, NP, 1) .* repmat(w1, 1, D) + ...
        diff_fit .* repmat(w2, 1, D) + ...
        diff_con .* repmat(w3, 1, D) + ...
        randn(NP,D) .* repmat(w4, 1, D);
    
    % Adaptive crossover rates
    CR = 0.5 + 0.3 * (1 - norm_cons) .* norm_fits;
    CR = repmat(CR, 1, D);
