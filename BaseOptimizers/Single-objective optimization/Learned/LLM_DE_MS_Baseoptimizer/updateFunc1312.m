% MATLAB Code
function [offspring] = updateFunc1312(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    % 7. Combined mutation with Gaussian noise
    mutants = popdecs + F .* (w.*d_elite + (1-w).*d_cons + 0.3*randn(NP,D).*d_rand);
    
    % 8. Rank-based crossover probability
