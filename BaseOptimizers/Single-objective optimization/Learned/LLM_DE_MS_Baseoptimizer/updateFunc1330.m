% MATLAB Code
function [offspring] = updateFunc1330(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    F = 0.4 + 0.3 * (1 - rank/NP) .* (1 - cons./max_cons);
    
    % 4. Generate mutant vectors with differential component
    idx = randperm(NP);
    a = idx(1:NP);
    b = idx(NP+1:2*NP);
    mutants = popdecs + F.*directions + 0.1*randn(NP,1).*(popdecs(a,:) - popdecs(b,:));
    ...
    violate = cons > 0;
    for i = 1:NP
        if violate(i) && rand() < 0.8
            offspring(i,:) = x_best + 0.6*(x_best - offspring(i,:));
        elseif violate(i)
