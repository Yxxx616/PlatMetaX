% MATLAB Code
function [offspring] = updateFunc1356(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    w_b = 1 ./ denom;          % Best component weight
    w_c = cv_norm ./ denom;    % Center component weight
    w_d = f_norm ./ denom;     % Diversity component weight
    
    % Calculate adaptive scaling factors
    F_b = 0.7 + 0.3 * (1 - f_norm);
    F_c = 0.5 * cv_norm;
    F_d = 0.3 * (1 - cv_norm);
    ...
    % Constraint-guided repair with adaptive perturbation
    repair_mask = cv_pos > 0;
    if any(repair_mask)
        beta = 0.5 * cv_norm(repair_mask);
        perturbation = 0.2 * (rand(sum(repair_mask),D) - 0.5) .* (ub - lb);
    ...
    for j = 1:D
        ...
        reflect_factor = 1 + 0.5*(cv_norm + f_norm);
    ...
    % Final feasibility enforcement with gradient-based reset
    out_of_bounds = any(offspring < lb(ones(NP,1),:) | offspring > ub(ones(NP,1),:), 2);
    if any(out_of_bounds)
        gradient = popdecs(out_of_bounds,:) - feasible_center;
        offspring(out_of_bounds,:) = feasible_center + 0.5*gradient + ...
                                   0.5*rand(sum(out_of_bounds),D).*(ub - lb);
    end
end