% MATLAB Code
function [offspring] = updateFunc1358(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    % Enhanced constraint repair with directional perturbation
    repair_mask = cv_pos > 0;
    if any(repair_mask)
        beta = 0.7 * cv_norm(repair_mask);
        direction = sign(rand(sum(repair_mask),D) - 0.5);
        perturbation = direction .* (ub - lb) .* (0.05 + 0.15 * rand(sum(repair_mask),D));
    ...
    % Final feasibility enforcement with directional reset
    out_of_bounds = any(offspring < lb(ones(NP,1),:) | offspring > ub(ones(NP,1),:), 2);
    if any(out_of_bounds)
        direction = sign(rand(sum(out_of_bounds),D) - 0.5);
        offspring(out_of_bounds,:) = feasible_center + ...
                                   0.3*direction.*(ub - lb).*rand(sum(out_of_bounds),D);
    end
end