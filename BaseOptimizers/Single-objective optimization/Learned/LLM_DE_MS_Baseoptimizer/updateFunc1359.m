% MATLAB Code
function [offspring] = updateFunc1359(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    if any(repair_mask)
        ...
        perturbation = direction .* (ub - lb) .* (0.1 * rand(sum(repair_mask),D));
    ...
    for j = 1:D
        ...
        reflect_factor = 1 + 0.3*(cv_norm + f_norm);
    ...
    if any(out_of_bounds)
        ...
        offspring(out_of_bounds,:) = feasible_center + ...
                                   0.2*direction.*(ub - lb).*rand(sum(out_of_bounds),D);
    end
end