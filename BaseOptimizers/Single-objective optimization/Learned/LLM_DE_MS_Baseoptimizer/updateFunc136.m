% MATLAB Code
function [offspring] = updateFunc136(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    ...
    % Select elite pool (top 30%) using combined scores
    combined_scores = 0.7 * norm_fits + 0.3 * (1 - norm_cons);
    ...
    weights = exp(-abs(popfits(elite_idx)) - abs(cons(elite_idx)));
    ...
    F = 0.5 + 0.3 * cos(pi * norm_fits) + 0.2 * norm_cons;
    ...
    CR = 0.9 - 0.3 * norm_fits .* norm_cons;
    CR = repmat(CR, 1, D);
