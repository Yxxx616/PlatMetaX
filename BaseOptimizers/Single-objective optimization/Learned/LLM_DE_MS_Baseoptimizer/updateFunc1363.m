% MATLAB Code
function [offspring] = updateFunc1363(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    w_feas = 1 ./ denom;
    w_best = (1 - cv_norm) ./ denom;
    w_div = f_norm ./ denom;
    
    % Vectorized mutation components
    diff_feas = feasible_center - popdecs;
    diff_best = x_best - popdecs;
    diff_div1 = popdecs(rand_idx(:,1),:) - popdecs(rand_idx(:,2),:);
    diff_div2 = popdecs(rand_idx(:,3),:) - popdecs(rand_idx(:,4),:);
    
    % Combine components
    v_feas = popdecs + F_feas.*diff_feas;
    v_best = popdecs + F_best.*diff_best;
    v_div = popdecs + F_div.*(diff_div1 + diff_div2);
    
    mutants = v_feas.*w_feas + v_best.*w_best + v_div.*w_div;
    
    % Constraint repair for high CV solutions
    repair_mask = cv_pos > 0.5;
    if any(repair_mask)
        beta = 0.5 * cv_norm(repair_mask);
        perturbation = (ub - lb) .* (rand(sum(repair_mask),D) - 0.5);
        mutants(repair_mask,:) = (1-beta).*mutants(repair_mask,:) + ...
                               beta.*(feasible_center + 0.1*perturbation);
    end
