% MATLAB Code
function [offspring] = updateFunc1377(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    mutants = popdecs + bsxfun(@times, F_best, diff_best) + ...
              bsxfun(@times, F_rand, diff_rand) + bsxfun(@times, F_feas, diff_feas);
