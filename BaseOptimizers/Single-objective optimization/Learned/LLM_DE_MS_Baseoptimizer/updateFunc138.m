% MATLAB Code
function [offspring] = updateFunc138(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    ...
    % Sort population by combined score
    ...
    v_cons = popdecs(r1,:) + ...
             F3 .* (popdecs(r2,:) - popdecs(r3,:)) .* repmat(1 - norm_cons, 1, D);
    ...
    CR = 0.9 * (1 - norm_cons) + 0.05 * norm_fits;
    CR = repmat(CR, 1, D);
