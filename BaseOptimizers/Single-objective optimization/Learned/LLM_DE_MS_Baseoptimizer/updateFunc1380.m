% MATLAB Code
function [offspring] = updateFunc1380(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    cv_norm = (cv_pos - cv_min) / (cv_max - cv_min + eps);
    ...
    % Adaptive weights
    w_best = 0.7 * (1 - cv_norm) .* (1 - f_norm);
    w_feas = 0.5 * cv_norm .* (1 - f_norm);
    w_div = 0.3 * (1 - cv_norm) .* f_norm;
