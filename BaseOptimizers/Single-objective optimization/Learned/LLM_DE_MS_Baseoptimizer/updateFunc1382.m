% MATLAB Code
function [offspring] = updateFunc1382(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    if any(out_of_bounds)
        ...
        offspring(out_of_bounds,:) = bsxfun(@plus, centroid, ...
                                   0.2 * bsxfun(@times, (ub - lb), rand(sum(out_of_bounds),D)-0.5));
    end
end