% MATLAB Code
function [offspring] = updateFunc1390(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    % Adaptive weights
    w_explore = 0.7 * (1 - f_norm) .* cv_norm;
    w_exploit = 0.3 * (1 - cv_norm) .* (1 - f_norm);
    ...
    % Enhanced mutation with adaptive F
    F = 0.4 + 0.4 * cv_norm;
    ...
    if any(out_of_bounds)
        offspring(out_of_bounds,:) = bsxfun(@plus, x_feas, ...
                                   0.05 * bsxfun(@times, (ub - lb), rand(sum(out_of_bounds),D)-0.5));
    end
end