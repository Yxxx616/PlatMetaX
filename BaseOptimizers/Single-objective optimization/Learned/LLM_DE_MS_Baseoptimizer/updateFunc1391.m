% MATLAB Code
function [offspring] = updateFunc1391(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    mutants = popdecs + bsxfun(@times, F .* w_cv, diff_best) + ...
              bsxfun(@times, F .* w_fit, diff_feas) + ...
              bsxfun(@times, F .* w_div, diff_rand);
    
    % Boundary handling with reflection
    reflect_scale = 0.3 + 0.2 * rand(NP, 1);
    for j = 1:D
