% MATLAB Code
function [offspring] = updateFunc1398(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    w_feas = 0.5 * (1 - cv_norm) + 0.25;
    w_qual = 0.5 * (1 - f_norm) + 0.25;
    ...
    if any(high_cv)
        offspring(high_cv,:) = bsxfun(@plus, x_feas, ...
                               0.3 * bsxfun(@times, (ub - lb), rand(sum(high_cv),D)-0.5));
    end
end