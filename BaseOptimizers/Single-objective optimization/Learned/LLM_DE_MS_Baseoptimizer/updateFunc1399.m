% MATLAB Code
function [offspring] = updateFunc1399(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    % Calculate adaptive weights (sum to 1)
    w_feas = 0.6 * (1 - cv_norm) + 0.2;
    w_qual = 0.6 * (1 - f_norm) + 0.2;
    ...
    % Generate random indices (2 distinct vectors per target)
    rand_idx = zeros(NP, 2);
    for i = 1:NP
    ...
    % Weighted mutation
    mutants = popdecs + F .* (w_feas .* diff_feas + w_qual .* diff_best + w_div .* diff_rand);
    ...
    % Diversity enhancement for highly constrained solutions
    high_cv = cv_norm > 0.8;
    if any(high_cv)
        n_high = sum(high_cv);
        offspring(high_cv,:) = bsxfun(@plus, x_feas, ...
                               bsxfun(@times, (ub - lb), rand(n_high,D)-0.5) .* 0.2);
    end
end