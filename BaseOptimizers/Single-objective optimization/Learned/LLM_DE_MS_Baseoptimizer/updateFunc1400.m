% MATLAB Code
function [offspring] = updateFunc1400(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    w_feas = 0.5 * (1 - cv_norm) + 0.2;
    w_qual = 0.5 * (1 - f_norm) + 0.2;
    ...
    if any(high_cv)
        ...
        offspring(high_cv,:) = bsxfun(@plus, x_feas, ...
                               bsxfun(@times, (ub - lb), rand(n_high,D)-0.5) .* 0.15);
    end
end