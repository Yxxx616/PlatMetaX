% MATLAB Code
function [offspring] = updateFunc1413(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    F_feas = 0.5 * (1 + tanh(1 - cv_norm));
    F_elite = 0.5 * (1 + tanh(1 - f_norm));
    F_div = 0.5 * (1 - tanh(cv_norm .* f_norm));
    ...
    % Combined mutation with adaptive scaling
    mutants = popdecs + F_feas.*diff_feas + F_elite.*diff_elite + F_div.*diff_div;
    ...
    for j = 1:D
        ...
        % Additional perturbation for boundary cases
    ...
    if any(worst_mask)
        ...
        offspring(worst_mask,:) = bsxfun(@plus, x_feas, ...
            bsxfun(@times, (ub - lb), rand(n_worst,D)-0.5) * 0.3);
    end
end