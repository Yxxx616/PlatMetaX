% MATLAB Code
function [offspring] = updateFunc1418(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    w_feas = 0.7 * (1 - cv_norm);
    w_elite = 0.2 * (1 - f_norm);
    ...
    % Dynamic crossover rate based on normalized constraints and fitness
    CR = 0.9 - 0.4 * cv_norm - 0.2 * f_norm;
