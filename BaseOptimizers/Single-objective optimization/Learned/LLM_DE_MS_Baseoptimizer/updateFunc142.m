% MATLAB Code
function [offspring] = updateFunc142(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    ...
    elite_size = max(2, floor(0.25 * NP));
    ...
    v(is_elite,:) = x_elite + F1(is_elite) .* (x_elite - x_poor) + ...
                   v_cons(is_elite,:) + v_div(is_elite,:);
    v(is_mod,:) = popdecs(is_mod,:) + v_cons(is_mod,:) + v_div(is_mod,:);
    v(~is_elite & ~is_mod,:) = popdecs(r3(~is_elite & ~is_mod),:) + ...
                              v_cons(~is_elite & ~is_mod,:);
    ...
    reflect_prob = repmat(0.5 + 0.4 * norm_fits, 1, D);
