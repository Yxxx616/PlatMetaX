% MATLAB Code
function [offspring] = updateFunc1422(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    w_feas = 0.5 * (1 - cv_norm) .* (1 + f_norm);
    w_elite = 0.3 * (1 - f_norm) .* (1 + cv_norm);
    ...
    F = 0.5;
    mutants = popdecs + F * (w_feas.*diff_feas + w_elite.*diff_elite + w_div.*diff_div);
    ...
    if any(poor_mask)
        ...
        offspring(poor_mask,:) = bsxfun(@plus, x_feas, ...
            bsxfun(@times, (ub - lb), rand(n_poor,D)-0.5) * 0.2);
    end
end