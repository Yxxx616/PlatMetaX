% MATLAB Code
function [offspring] = updateFunc1423(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    w_feas = 0.4 * (1 - cv_norm) .* (1 + f_norm);
    w_elite = 0.4 * (1 - f_norm) .* (1 + cv_norm);
    ...
    % Compute feasible center (weighted by constraint violation)
    feasible_mask = cons <= 0;
    if any(feasible_mask)
        weights = 1./(1 + cv_pos(feasible_mask));
        weights = weights / sum(weights);
        x_feas = sum(popdecs(feasible_mask,:) .* weights, 1);
    else
        weights = 1./(1 + cv_pos);
        weights = weights / sum(weights);
    ...
    % Combined mutation with adaptive weights and scaling factor F=0.5
    F = 0.5;
    eta = 0.1 * (1 + f_norm + cv_norm);
    noise = randn(NP, D) .* eta;
    mutants = popdecs + F * (w_feas.*diff_feas + w_elite.*diff_elite + w_div.*diff_div) + noise;
