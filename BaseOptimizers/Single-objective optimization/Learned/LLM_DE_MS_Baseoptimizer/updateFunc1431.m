% MATLAB Code
function [offspring] = updateFunc1431(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    F_elite = 0.7 * (1 - 0.6*f_norm - 0.4*cv_norm);
    F_feas = 0.5 * (1 - 0.4*f_norm + 0.6*cv_norm);
    F_div = 0.3 * (1 + 0.2*f_norm - 0.3*cv_norm);
    F_rand = 0.1 * (0.5 + 0.5*cv_norm);
    
    % Compute weights using softmax with temperature
    T = [0.25, 0.35, 0.45, 0.55]; % Adjusted temperatures
