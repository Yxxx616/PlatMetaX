% MATLAB Code
function [offspring] = updateFunc1432(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    F_elite = 0.8 * (1 - 0.7*f_norm - 0.3*cv_norm);
    F_feas = 0.6 * (1 - 0.5*f_norm + 0.5*cv_norm);
    F_div = 0.4 * (1 + 0.3*f_norm - 0.2*cv_norm);
    F_rand = 0.2 * (0.5 + 0.5*cv_norm);
    
    % Compute weights using softmax with temperature
    T = [0.2, 0.3, 0.4, 0.5]; % Adjusted temperatures
    ...
    high_cv = cv_norm > 0.75;
    if any(high_cv)
        n_high = sum(high_cv);
        offspring(high_cv,:) = x_feas + (ub - lb).*(rand(n_high,D)-0.5)*0.25;
    end
end