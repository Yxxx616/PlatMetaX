% MATLAB Code
function [offspring] = updateFunc1433(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    F_elite = 0.9 * (1 - 0.6*f_norm - 0.4*cv_norm);
    F_feas = 0.7 * (1 - 0.4*f_norm + 0.6*cv_norm);
    F_div = 0.5 * (1 + 0.2*f_norm - 0.3*cv_norm);
    F_rand = 0.3 * (0.4 + 0.6*cv_norm);
    
    % Compute weights using improved softmax
    T = [0.15, 0.25, 0.35, 0.45]; % Adjusted temperatures
    w = exp(-(f_norm + 0.8*cv_norm)./T);
    w = w ./ sum(w, 2);
    ...
    CR_base = 0.9;
    CR = CR_base - 0.5*cv_norm - 0.4*f_norm;
    CR = min(max(CR, 0.2), 0.95);
