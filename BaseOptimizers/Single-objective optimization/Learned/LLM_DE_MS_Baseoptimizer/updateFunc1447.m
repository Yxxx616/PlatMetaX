% MATLAB Code
function [offspring] = updateFunc1447(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    % Adaptive weights
    w_elite = 0.5./(1 + f_norm + cv_norm);
    w_feas = 0.3./(1 + cv_norm);
    w_div = 0.2./(1 + f_norm);
    ...
    % Dynamic scaling factor (adaptive based on constraints)
    F = 0.5 * (1 - cv_norm) + 0.3;
