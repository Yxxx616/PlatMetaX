% MATLAB Code
function [offspring] = updateFunc1462(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    range = ub - lb;
    ...
    mutants = popdecs + F.*elite_diff + F.*best_diff + 0.7*F.*rand_diff;
