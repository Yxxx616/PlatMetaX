% MATLAB Code
function [offspring] = updateFunc1466(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    range = ub - lb;
    ...
    F2 = 0.3 + 0.2*w + 0.1*randn(NP,1);
    F3 = 0.2*(1+w) + 0.1*randn(NP,1);
    ...
    % Adaptive sigma based on fitness diversity
    sigma = 0.2*(1-w(top_idx)).*range .* (1 + 0.5*randn(top_N,D));
