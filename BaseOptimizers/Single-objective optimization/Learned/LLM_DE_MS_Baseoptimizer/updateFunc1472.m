% MATLAB Code
function [offspring] = updateFunc1472(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    F_elite = 0.7*(1 - w) + 0.1*rand(NP,1);
    F_best = 0.5*w + 0.1*rand(NP,1);
    F_opp = 0.3*(1 - w).*rand(NP,1);
    F_rand = 0.2*w.*rand(NP,1);
    ...
    % Constraint-driven refinement for top solutions
    ...
    local_search = min(max(local_search, ub), lb);
