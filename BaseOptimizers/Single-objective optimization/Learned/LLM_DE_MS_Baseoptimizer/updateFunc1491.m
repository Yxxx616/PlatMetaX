% MATLAB Code
function [offspring] = updateFunc1491(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    % Mutation
    mutants = x_base + alpha .* weighted_diff;
    ...
    % Local refinement for top 10% solutions
    [~, sorted_idx] = sort(popfits + 1e6*cv);
    refine_N = max(1, round(0.1*NP));
    refine_idx = sorted_idx(1:refine_N);
    
    sigma = 0.02 * (1 - norm_cv(refine_idx)) .* (ub-lb) .* randn(refine_N,D);
