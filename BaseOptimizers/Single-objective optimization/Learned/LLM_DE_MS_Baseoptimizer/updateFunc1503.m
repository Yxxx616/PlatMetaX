% MATLAB Code
function [offspring] = updateFunc1503(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    perturbation = F3 .* randn(NP, D) .* (1 - norm_fits);
    ...
    sigma = 0.1 * (ub - lb) .* (1 - norm_fits(refine_idx)).^2;
