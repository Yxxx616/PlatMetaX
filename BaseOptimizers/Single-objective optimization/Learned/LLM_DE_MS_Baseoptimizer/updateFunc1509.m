% MATLAB Code
function [offspring] = updateFunc1509(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    perturbation = F3 .* randn(NP, D) .* norm_cv(:, ones(1,D));
    
    mutants = popdecs + F1.*elite_diff + F2.*rand_diff.*(1 - norm_fits(:, ones(1,D))) + perturbation;
