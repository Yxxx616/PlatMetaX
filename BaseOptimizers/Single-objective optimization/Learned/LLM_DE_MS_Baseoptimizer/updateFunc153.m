% MATLAB Code
function [offspring] = updateFunc153(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    offspring = zeros(NP, D);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    
    % 1. Weight calculation with median fitness
    ...
    F = 0.5 + 0.2 * cos(pi * abs(cons)/max_cons);
    CR = 0.9 - 0.5 * tanh(2 * abs(cons)/max_cons);
    ...
    % 5. Direction vectors with exploration-exploitation balance
