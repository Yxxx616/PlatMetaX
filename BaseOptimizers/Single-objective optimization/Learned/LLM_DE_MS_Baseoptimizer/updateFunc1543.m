% MATLAB Code
function [offspring] = updateFunc1543(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    % 3. Calculate constraint-aware direction
    feasible = cons <= 0;
    if any(feasible)
