% MATLAB Code
function [offspring] = updateFunc1547(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    offspring = zeros(NP, D);
    
    % 1. Calculate adaptive weights based on constraints
    abs_cons = abs(cons);
    mu_c = mean(abs_cons);
    sigma_c = std(abs_cons);
    alpha = 1 ./ (1 + exp(-5 * (abs_cons - mu_c) ./ max(1e-10, sigma_c));
