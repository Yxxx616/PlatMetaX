% MATLAB Code
function [offspring] = updateFunc1551(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    % 6. Adaptive mutation with scaling factor F
    F = 0.7;
    mutants = popdecs + F * (w_feas.*feas_dir + (1-w_feas).*(0.5*elite_dir + 0.5*opposite_dir));
    ...
    sigma_top = 0.02 * (ub - lb) .* (1 - w_feas(top_idx));
