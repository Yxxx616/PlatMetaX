% MATLAB Code
function [offspring] = updateFunc156(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    ...
    F = 0.5 + 0.3 * tanh(1 - norm_cons) .* (1 - ranks);
    CR = 0.9 - 0.5 * norm_cons.^2;
    
    % 3. Elite selection (top 30%)
    [~, sorted_idx] = sort(popfits);
    elite_idx = sorted_idx(1:ceil(0.3*NP));
    
    % 4. Generate random indices (6 distinct indices per individual)
    rand_idx = zeros(NP, 6);
    for i = 1:NP
    ...
    p_explore = 0.4; % Fixed exploration probability
    explore_mask = rand(NP,1) < p_explore;
    d = zeros(NP, D);
    
    % Exploration direction (DE/rand/3)
    r1 = rand_idx(:,1); r2 = rand_idx(:,2);
    r3 = rand_idx(:,3); r4 = rand_idx(:,4);
    r5 = rand_idx(:,5); r6 = rand_idx(:,6);
    d(exploit_mask,:) = (popdecs(r1(exploit_mask),:) - popdecs(r2(exploit_mask),:) + ...
                        popdecs(r3(exploit_mask),:) - popdecs(r4(exploit_mask),:) + ...
                        popdecs(r5(exploit_mask),:) - popdecs(r6(exploit_mask),:)) / 3;
