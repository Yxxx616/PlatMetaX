% MATLAB Code
function [offspring] = updateFunc1568(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    % 7. Adaptive scaling factors (using sine function for smoother transition)
    F = 0.5 + 0.2 * sin(pi * w_cons);
    
    % 8. Differential vectors
    diff_vec = popdecs(r1,:) - popdecs(r2,:);
    
    % 9. Mutation with adjusted weights
    mutants = x_base + bsxfun(@times, F, diff_vec) + ...
              0.6 * bsxfun(@times, w_fit, d_best) + ...
              0.4 * bsxfun(@times, w_cons, d_feas);
