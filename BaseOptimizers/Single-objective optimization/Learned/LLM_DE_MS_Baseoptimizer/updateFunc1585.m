% MATLAB Code
function [offspring] = updateFunc1585(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    % 2. Identify elite solutions (top 10%)
    cf = popfits + 1e6*max(0, cons);
    [~, sorted_idx] = sort(cf);
    elite_pool = popdecs(sorted_idx(1:ceil(NP*0.1)), :);
