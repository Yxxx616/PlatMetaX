% MATLAB Code
function [offspring] = updateFunc1587(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    alpha = 0.7 + 0.2*rand(NP,1);
    beta = 0.3 + 0.2*rand(NP,1);
    gamma = 0.5 + 0.3*rand(NP,1);
    delta = 0.3 + 0.2*rand(NP,1);
    ...
    for i = 1:NP
        if feasible(i)
            dir_vectors(i,:) = alpha(i)*(elite - popdecs(i,:)) + beta(i)*(diverse - popdecs(i,:));
        else
            dir_vectors(i,:) = gamma(i)*(centroid - popdecs(i,:)) + ...
                              delta(i)*(popdecs(rand_idx1(i),:) - popdecs(rand_idx2(i),:));
        end
    end
