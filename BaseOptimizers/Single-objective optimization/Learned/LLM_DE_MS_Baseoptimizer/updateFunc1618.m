% MATLAB Code
function [offspring] = updateFunc1618(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    
    % 1. Create elite pool (top 20% considering constraints)
    ...
    for i = 1:NP
        ...
        pert_factor = 0.5 - 0.3 * (c_abs(i) / (c_max + eps));
        perturbation = pert_factor * (popdecs(r1,:) - popdecs(r2,:)) .* (1 + c_abs(i)/(c_max + eps));
