% MATLAB Code
function [offspring] = updateFunc1641(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    elite_size = max(2, ceil(NP*0.15));
    worst_size = max(2, ceil(NP*0.15));
    ...
    % Adaptive parameters with enhanced exploration
    F1 = 0.6 + 0.2*(1 - ranks./NP);
    F2 = 0.4 * (f_max - popfits)./f_range;
    F3 = 0.2 * randn(NP, 1);
    ...
    for i = 1:NP
        ...
        CR = 0.85 * (1 - ranks(i)/NP)^2 * (1 - c_abs(i)/c_max);
        mask = rand(1,D) < CR;
        j_rand = randi(D);
        mask(j_rand) = true;
