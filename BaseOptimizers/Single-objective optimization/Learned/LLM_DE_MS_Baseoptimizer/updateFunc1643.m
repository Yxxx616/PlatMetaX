% MATLAB Code
function [offspring] = updateFunc1643(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    F1 = 0.8 + 0.1*(1 - ranks./NP);
    F2 = 0.6 * (f_max - popfits)./f_range;
    F3 = 0.4 * randn(NP, 1);
    ...
    for i = 1:NP
        ...
        CR = 0.95 * (1 - ranks(i)/NP)^1.2 * (1 - c_abs(i)/c_max);
        mask = rand(1,D) < CR;
        j_rand = randi(D);
        mask(j_rand) = true;
