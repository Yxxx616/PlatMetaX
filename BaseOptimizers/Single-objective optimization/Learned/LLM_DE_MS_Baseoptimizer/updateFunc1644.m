% MATLAB Code
function [offspring] = updateFunc1644(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    % 2. Create elite pool (top 25%)
    elite_size = max(2, ceil(NP*0.25));
    ...
    beta = (popfits - f_min)./f_range;  % Fitness ratio
    
    % 4. Generate offspring (vectorized)
    ...
    for i = 1:NP
        ...
        % Composite mutation
        mutation = popdecs(i,:) + ...
                 F1*alpha(i)*(elite - popdecs(i,:)) + ...
                 F2*beta(i)*(popdecs(r1,:) - popdecs(r2,:)) + ...
                 F3*(1-beta(i))*(popdecs(r3,:) - x_best);
        
        % Adaptive crossover
        CR = 0.9*(1 - ranks(i)/NP)^0.5 * alpha(i);
        mask = rand(1,D) < CR;
        j_rand = randi(D);
        mask(j_rand) = true;
