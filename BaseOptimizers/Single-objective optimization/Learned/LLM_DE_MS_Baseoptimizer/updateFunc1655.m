% MATLAB Code
function [offspring] = updateFunc1655(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    combined_score = popfits + 100*norm_cons; % Strong penalty for constraints
    ...
    for i = 1:NP
        ...
        F1 = 0.5 * (1 - norm_cons(i)); % More exploitation for feasible solutions
        F2 = 0.5 * norm_cons(i); % More exploration for constrained solutions
        F3 = 0.3 + 0.4 * rand(); % Random component
