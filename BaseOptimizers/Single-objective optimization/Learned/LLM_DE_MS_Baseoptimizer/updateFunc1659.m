% MATLAB Code
function [offspring] = updateFunc1659(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    offspring(viol_low) = lb(viol_low) + rand(sum(viol_low(:)),1) .* ...
                         (min(popdecs(:,viol_low)) - lb(viol_low));
    offspring(viol_high) = ub(viol_high) - rand(sum(viol_high(:)),1) .* ...
                          (ub(viol_high) - max(popdecs(:,viol_high)));
    
    % Final random perturbation (5% probability)
