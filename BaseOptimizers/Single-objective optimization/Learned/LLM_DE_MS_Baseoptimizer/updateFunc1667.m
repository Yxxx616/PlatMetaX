% MATLAB Code
function [offspring] = updateFunc1667(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    rand_idx = rand_idx(:, 2:4); % Get 3 random indices
    ...
    for i = 1:NP
        ...
        % Enhanced mutation with multiple directions
        mutation = popdecs(i,:) + ...
                  F1 .* (x_best - popdecs(i,:)) + ...
    ...
    % Reflection probability based on fitness and constraints
    reflect_prob = 0.6 * (1 - norm_cons) + 0.4 * norm_fits;
