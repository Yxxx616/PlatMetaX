% MATLAB Code
function [offspring] = updateFunc1671(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    % Compute weighted feasible average if any feasible solutions exist
    if num_feasible > 0
    ...
    for i = 1:NP
        ...
        % Adaptive parameters
        F1 = 0.5 * (1 - norm_cons(i)) + 0.3 * norm_fits(i);
        F2 = 0.2 + 0.3 * norm_cons(i);
        F3 = 0.5 * norm_cons(i);
