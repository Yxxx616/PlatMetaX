% MATLAB Code
function [offspring] = updateFunc1689(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    for i = 1:NP
        ...
        % Adaptive mutation
        mutation = x_base + 0.8 * diff_vector + ...
                  0.5 * w_cons(i) * best_dir + ...
                  0.3 * (1-w_fit(i)) * rand_perturb;
        
        % Exponential crossover with adaptive length
        j_rand = randi(D);
        L = 1 + floor(exp(-0.5*(w_fit(i)+w_cons(i))) * D);
        mask = false(1,D);
    ...
    offspring(perturb_mask) = offspring(perturb_mask) + ...
                            0.02 * (ub(perturb_mask) - lb(perturb_mask)) .* randn(sum(perturb_mask(:)),1);
    offspring = min(max(offspring, lb), ub);
end