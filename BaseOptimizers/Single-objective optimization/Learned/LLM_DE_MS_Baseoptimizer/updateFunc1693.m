% MATLAB Code
function [offspring] = updateFunc1693(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    % Combined weights (70% focus on constraints)
    alpha = 0.7;
    ...
    for i = 1:NP
        ...
        mutation = x_base + F(i)*(popdecs(r(3),:) - popdecs(r(4),:)) + ...
                  0.6*weights(i)*d_best + 0.4*(1-w_cons(i))*d_feas;
    ...
    % Final adaptive perturbation (only for worst solutions)
    perturb_mask = rand(NP,D) < repmat(0.2*weights,1,D);
