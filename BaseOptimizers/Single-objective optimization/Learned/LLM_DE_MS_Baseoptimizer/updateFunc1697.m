% MATLAB Code
function [offspring] = updateFunc1697(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    % Adaptive parameters
    F = 0.5 * (1 + cos(pi * w_cons)) .* (0.3 + 0.4 * w_fit);
    CR = 0.1 + 0.7 * (1 - w_fit) .* (1 - 0.3 * w_cons);
    j_rand = randi(D, NP, 1);
    ...
    for i = 1:NP
        ...
        % Enhanced hybrid mutation
        mutation = base_vec + F(i) * (popdecs(r(1),:) - popdecs(r(2),:)) + ...
                  0.7 * (1 - w_cons(i)) * (popdecs(r(3),:) - popdecs(r(4),:));
        ...
        offspring(i,viol_low) = lb(viol_low) + 0.4 * (base_vec(viol_low) - lb(viol_low));
        offspring(i,viol_high) = ub(viol_high) - 0.4 * (ub(viol_high) - base_vec(viol_high));
    end
    
    % Adaptive perturbation for highly constrained solutions
    perturb_mask = rand(NP,D) < repmat(0.03 * (1 + w_cons), 1, D);
