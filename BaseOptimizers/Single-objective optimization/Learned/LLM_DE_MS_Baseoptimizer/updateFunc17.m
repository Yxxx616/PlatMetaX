% MATLAB Code
function [offspring] = updateFunc17(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    ...
    min_cons = min(cons);
    max_cons = max(cons);
    cons_norm = (cons - min_cons) / (max_cons - min_cons + 1e-12);
    ...
    for i = 1:NP
        ...
        % Select two distinct random vectors (excluding current)
