% MATLAB Code
function [offspring] = updateFunc1704(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    % Adaptive parameters
    alpha = 0.7 + 0.2 * (1 - mean(norm_cons));  % Blend between best and best feasible
    F = 0.5 + 0.3 * (1 - norm_fits) .* (1 - norm_cons);
    beta = 0.3 * (1 - norm_cons);
    CR = 0.9 - 0.3 * norm_fits;
    ...
    for i = 1:NP
        ...
        % Enhanced mutation with constraint guidance
    ...
    % Small perturbation for diversity on worst solutions
    worst_mask = (norm_fits + norm_cons) > 0.8;
    if any(worst_mask)
        perturb_amount = 0.1 * (lb + (ub-lb).*rand(sum(worst_mask),D));
        offspring(worst_mask,:) = offspring(worst_mask,:) + perturb_amount;
