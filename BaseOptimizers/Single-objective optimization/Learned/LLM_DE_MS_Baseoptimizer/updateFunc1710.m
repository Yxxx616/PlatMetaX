% MATLAB Code
function [offspring] = updateFunc1710(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    % Adaptive parameters
    alpha = 0.7 * (1 - norm_cons) + 0.1;
    F_f = 0.5 + 0.4 * (1 - norm_fits);
    F_c = 0.3 * norm_cons;
    CR = 0.9 - 0.6 * norm_fits;
    ...
    poor_sols = (norm_fits + norm_cons) > 1.5;
    if any(poor_sols)
