% MATLAB Code
function [offspring] = updateFunc1711(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    poor_sols = (norm_fits + norm_cons) > 1.2;
    if any(poor_sols)
        perturb_amount = 0.1 * (rand(sum(poor_sols),D) - 0.5) .* (ub - lb);
