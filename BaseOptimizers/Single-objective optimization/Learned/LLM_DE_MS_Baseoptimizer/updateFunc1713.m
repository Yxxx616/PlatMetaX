% MATLAB Code
function [offspring] = updateFunc1713(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    if any(poor_sols)
        perturb_amount = 0.15 * (rand(sum(poor_sols),D) - 0.5) .* (ub - lb);
