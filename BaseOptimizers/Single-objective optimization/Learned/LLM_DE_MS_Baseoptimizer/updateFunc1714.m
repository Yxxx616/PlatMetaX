% MATLAB Code
function [offspring] = updateFunc1714(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    % Adaptive parameters
    alpha = 0.6 * (1 - norm_cons) + 0.2;  % Blend between best and feasible best
    F_f = 0.5 + 0.4 * (1 - norm_fits);    % Stronger scaling for worse solutions
    F_c = 0.3 * norm_cons;                % Constraint influence increases with violation
    CR = 0.85 - 0.5 * norm_fits;          % Higher crossover for worse solutions
    ...
    % Enhanced diversity maintenance for poor solutions
    poor_sols = (norm_fits > 0.8) | (norm_cons > 0.8);
    if any(poor_sols)
        perturb_amount = 0.2 * (rand(sum(poor_sols),D) - 0.5) .* (ub - lb);
        offspring(poor_sols,:) = offspring(poor_sols,:) + perturb_amount;
