% MATLAB Code
function [offspring] = updateFunc1715(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    % Adaptive parameters
    alpha = 0.5 * (1 + tanh(5*(0.5 - norm_cons)));  % Smooth transition between best and feasible best
    F_f = 0.5 + 0.3 * tanh(2*(popfits - f_avg)/(f_max - f_min + eps)); % Fitness-based scaling
    F_c = 0.3 * (1 - exp(-2 * norm_cons));          % Constraint-based scaling
    CR = 0.9 - 0.5 * norm_fits;                     % Adaptive crossover rate
    ...
    for i = 1:NP
        ...
        % Select based on fitness rank
        [~, sorted_fit] = sort(popfits(available));
        prob_fit = exp(-4*(1:length(available))/length(available));
        idx_matrix(i,1:2) = available(datasample(1:length(available), 2, 'Weights', prob_fit));
        
        % Select based on constraint rank
        [~, sorted_cons] = sort(pos_cons(available));
        prob_cons = exp(-4*(1:length(available))/length(available));
        idx_matrix(i,3:4) = available(datasample(1:length(available), 2, 'Weights', prob_cons));
    end
    ...
    if any(high_constrained)
        ...
        perturb_dir = perturb_dir ./ sqrt(sum(perturb_dir.^2, 2)); % Normalize
