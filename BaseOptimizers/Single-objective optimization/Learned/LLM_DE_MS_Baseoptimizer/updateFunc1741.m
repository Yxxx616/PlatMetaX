% MATLAB Code
function [offspring] = updateFunc1741(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    % Select target vector (best feasible or least infeasible)
    ...
    % Feasible direction component
    if any(feasible_mask)
        feas_diff = popdecs(feasible_mask,:) - target_vec;
        d_f = mean(feas_diff, 1);
    else
        d_f = zeros(1, D);
    end
    
    % Constraint-driven component
    if any(infeasible_mask)
        infeas_diff = popdecs(infeasible_mask,:) - target_vec;
        weights = pos_cons(infeasible_mask) / sum(pos_cons(infeasible_mask));
        d_c = sum(infeas_diff .* weights(:, ones(1,D)), 1);
    else
        d_c = zeros(1, D);
    end
    ...
    mutation = target_rep + ...
               ...
               F_m(:, ones(1,D)) .* (popdecs(r1,:) - popdecs(r2,:));
