% MATLAB Code
function [offspring] = updateFunc1750(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    offspring(lower_violation) = 0.5*(lb_rep(lower_violation) + popdecs(lower_violation));
    offspring(upper_violation) = 0.5*(ub_rep(upper_violation) + popdecs(upper_violation));
    
    % Elite preservation with enhanced strategy
    if any(feasible_mask)
        [~, sort_idx] = sort(popfits(feasible_mask));
        preserve_num = min(3, sum(feasible_mask));
        offspring(feas_idx(sort_idx(1:preserve_num)),:) = popdecs(feas_idx(sort_idx(1:preserve_num)),:);
    else
        [~, sort_idx] = sort(pos_cons);
        preserve_num = min(3, NP);
