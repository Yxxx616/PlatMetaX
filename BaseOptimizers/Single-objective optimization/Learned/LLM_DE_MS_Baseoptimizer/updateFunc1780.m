% MATLAB Code
function [offspring] = updateFunc1780(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    % Enhanced random perturbation for diversity
    if rand() < 0.1
        perturb_num = max(1, floor(0.05*NP));
        perturb_idx = randperm(NP, perturb_num);
        offspring(perturb_idx,:) = lb + (ub-lb).*rand(perturb_num,D);
    end
end