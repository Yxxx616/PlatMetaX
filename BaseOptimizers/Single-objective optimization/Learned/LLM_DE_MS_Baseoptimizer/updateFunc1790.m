% MATLAB Code
function [offspring] = updateFunc1790(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    F1 = 0.7 + 0.2 * rand(NP, 1);
    F2 = 0.4 * feasible_ratio * ones(NP, 1);
    F3 = 0.3 * (1 - norm_cons);
    F4 = 0.1 * rand(NP, 1);
    F5 = 0.05 * rand(NP, 1);
    ...
    preserve_num = max(1, min(3, floor(NP*0.05)));
    if any(feasible_mask)
    ...
    % Adaptive diversity injection when feasible ratio is low
    if feasible_ratio < 0.3
        perturb_idx = randperm(NP, max(1, floor(NP*0.1)));
        offspring(perturb_idx,:) = offspring(perturb_idx,:) + ...
                                 randn(length(perturb_idx),D) .* (ub-lb)*0.01;
    end
end