% MATLAB Code
function [offspring] = updateFunc1796(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    % Dynamic perturbation when diversity is low
    if feasible_ratio < 0.4
        std_pop = std(popdecs);
        diversity = mean(std_pop);
        if diversity < 0.2 * mean(ub-lb)
            perturb_num = max(1, floor(NP*0.4));
            perturb_idx = randperm(NP, perturb_num);
            offspring(perturb_idx,:) = offspring(perturb_idx,:) + ...
                randn(perturb_num,D) .* (ub-lb)*0.1;
        end
    end
end