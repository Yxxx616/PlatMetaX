% MATLAB Code
function [offspring] = updateFunc1800(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    % Enhanced adaptive scaling factors with novel components
    F1 = 0.8 * (1 - norm_cons) + 0.2 * rand(NP, 1);
    F2 = 0.5 * feasible_ratio * ones(NP, 1);
    F3 = 0.4 * (1 - norm_cons).^2;
    F4 = 0.3 * (0.5 + 0.5*rand(NP, 1));
    F5 = 0.2 * (1 - feasible_ratio) .* rand(NP, 1);
    F6 = 0.1 * (1 - feasible_ratio) .* rand(NP, 1);
    F7 = 0.15 * (1 + norm_cons) .* rand(NP, 1);  % Novel adaptive perturbation
    ...
    % New components
    diversity_component = (elite_rep - feas_mean_rep) .* rand(NP, D);
    perturbation_component = (elite_rep - popdecs(r3,:)) .* (1 + norm_cons(:, ones(1,D)));
