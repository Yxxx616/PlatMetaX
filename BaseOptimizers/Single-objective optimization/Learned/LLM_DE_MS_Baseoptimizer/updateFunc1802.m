% MATLAB Code
function [offspring] = updateFunc1802(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    F4 = 0.4 * (0.5 + 0.5*rand(NP, 1));
    F5 = 0.3 * (1 - feasible_ratio) .* rand(NP, 1);
    F6 = 0.2 * (1 + norm_cons) .* rand(NP, 1);
    F7 = 0.1 * (1 - feasible_ratio) .* rand(NP, 1);
    ...
    % Novel components
    diversity_component = (elite_rep - feas_mean_rep) .* randn(NP, D);
    ...
    mutation = elite_rep + ...
               ...
               F6(:, ones(1,D)) .* perturbation_component + ...
               F7(:, ones(1,D)) .* diversity_component;
    ...
    offspring(lower_violation) = lb_rep(lower_violation) + ...
        rand(sum(lower_violation(:)),1) .* (ub_rep(lower_violation) - lb_rep(lower_violation));
    offspring(upper_violation) = ub_rep(upper_violation) - ...
        rand(sum(upper_violation(:)),1) .* (ub_rep(upper_violation) - lb_rep(upper_violation));
