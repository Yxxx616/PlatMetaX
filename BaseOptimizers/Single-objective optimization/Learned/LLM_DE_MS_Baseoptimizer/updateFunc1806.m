% MATLAB Code
function [offspring] = updateFunc1806(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    F1 = 0.7 + 0.1 * rand(NP, 1);  % Stronger elite guidance
    F2 = 0.5 * feasible_ratio * ones(NP, 1);
    F3 = 0.4 * (1 - norm_cons).^2;
    F4 = 0.3 + 0.2 * rand(NP, 1);  % More exploration
    F5 = 0.2 * (1 - norm_fits) .* rand(NP, 1);
    ...
    % Adaptive crossover with improved CR
    ...
    % Improved elite preservation (top 10% solutions)
