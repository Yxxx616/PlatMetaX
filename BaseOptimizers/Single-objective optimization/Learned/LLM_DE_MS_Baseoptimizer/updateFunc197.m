% MATLAB Code
function [offspring] = updateFunc197(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    ...
    for i = 1:NP
        ...
        F1 = 0.7 + 0.1 * tanh((popfits(i) - avg_fit)/std_fit);
        F2 = 0.5 * (1 - abs(cons(i))/sum_con);
        F3 = 0.3 * (1 - (popfits(i) - min_fit)/(max_fit - min_fit + eps));
    ...
    perturbation = 0.008*(max_fit - min_fit)/(std_fit + eps) * randn(size(offspring)).*(ub-lb);
