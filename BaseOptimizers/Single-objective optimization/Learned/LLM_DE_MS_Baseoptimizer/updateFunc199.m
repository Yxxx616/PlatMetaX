% MATLAB Code
function [offspring] = updateFunc199(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    ...
    for i = 1:NP
        ...
        F3 = 0.1 * tanh((popfits(i) - avg_fit)/(std_fit));
    ...
    perturbation = 0.02*(max_fit - min_fit)/(std_fit + eps) * randn(size(offspring));
