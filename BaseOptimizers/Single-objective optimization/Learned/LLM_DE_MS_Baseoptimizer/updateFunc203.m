% MATLAB Code
function [offspring] = updateFunc203(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    ...
    for i = 1:NP
        ...
        F2 = 0.6 * tanh((popfits(i) - avg_fit)/std_fit);
        F3 = 0.4 * abs(cons(i))/sum_con;
        
        % Mutation components
        elite_diff = elite - popdecs(i, :);
        
        rank_diff = (top_rank(r1(i), :) - top_rank(s1(i), :)) + ...
                   (mid_rank(r2(i), :) - mid_rank(s2(i), :)) + ...
                   (bot_rank(r3(i), :) - bot_rank(s3(i), :));
    ...
    % Final small perturbation
    perturbation_scale = 0.05 * (max(popfits) - min(popfits))/(std_fit + eps);
