% MATLAB Code
function [offspring] = updateFunc220(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    ...
    % Calculate adaptive mutation factors (improved)
    F1 = 0.8 * (1 - abs_cons/max_con);
    F2 = 0.6 * tanh(norm_fits) .* (1 - abs_cons/sum_con);
    F3 = 0.4 * diversity/D * ones(NP,1);
    ...
    % Improved rank-based adaptive combination
