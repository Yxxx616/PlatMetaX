% MATLAB Code
function [offspring] = updateFunc230(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    ...
    % Vectorized mutation
    for i = 1:NP
        % Component 1: Elite direction
        d1 = elite - popdecs(i,:);
        
        % Component 2: Feasible direction
        d2 = best_feas - popdecs(i,:);
        
        % Component 3: Diversity maintenance
        d3 = center - popdecs(i,:);
        
        % Adaptive weights
        w1 = f_weights(i);
        w2 = c_weights(i);
        w3 = 0.1;  % Fixed small weight for diversity
