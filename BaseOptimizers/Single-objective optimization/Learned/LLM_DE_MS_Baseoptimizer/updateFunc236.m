% MATLAB Code
function [offspring] = updateFunc236(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    ...
    c_weights = 0.3 * (abs_cons - c_min) ./ (c_max - c_min);
    r_weights = 0.2 * rand(NP, 1);
    ...
    offspring = offspring.*(~out_low & ~out_high) + ...
               (lb + rand(NP,D).*(popdecs - lb)).*out_low + ...
               (ub - rand(NP,D).*(ub - popdecs)).*out_high;
