% MATLAB Code
function [offspring] = updateFunc246(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    ...
    % Calculate adaptive weights with new coefficients
    f_weights = 0.5 * (1 - (popfits - f_min) ./ (f_max - f_min));
    c_weights = 0.3 * (1 - (abs_cons - c_min) ./ (c_max - c_min));
    r_weights = 0.2 * (1 + abs_cons ./ c_max) .* rand(NP, 1);
