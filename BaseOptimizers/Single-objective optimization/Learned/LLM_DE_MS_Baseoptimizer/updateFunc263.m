% MATLAB Code
function [offspring] = updateFunc263(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    ...
    f_weights = 0.7 * (1 - (popfits - f_min) ./ (f_max - f_min)) + 0.1 * rand(NP, 1);
    c_weights = 0.5 * (1 - (abs_cons - c_min) ./ (c_max - c_min)) + 0.1 * rand(NP, 1);
    r_weights = 0.3 * (1 - abs_cons ./ c_max);
    ...
    % Enhanced mutation with perturbation
    offspring = popdecs + f_weights.*d_elite + c_weights.*d_feas + ...
                r_weights.*d_rand .* (1 + 0.01*randn(NP, D));
