% MATLAB Code
function [offspring] = updateFunc273(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    ...
    % Calculate adaptive weights with improved balance
    f_weights = 0.4 * (1 - (popfits - f_min) ./ (f_max - f_min)) + 0.1 * rand(NP, 1);
    c_weights = 0.3 * (1 - (abs_cons - c_min) ./ (c_max - c_min)) + 0.1 * rand(NP, 1);
    r_weights = 0.2 * (1 - abs_cons ./ c_max);
    cons_weights = 0.3 * (abs_cons ./ c_max);
    F = 0.5 + 0.15 * randn(NP, 1);
    
    % Vectorized mutation operation with enhanced components
    ...
    offspring = popdecs + f_weights.*d_elite + c_weights.*d_feas + ...
                F.*r_weights.*d_rand + cons_weights.*d_cons;
    ...
    perturb = 0.01 * randn(NP, D) .* (ub - lb) .* (1 - abs_cons./c_max);
