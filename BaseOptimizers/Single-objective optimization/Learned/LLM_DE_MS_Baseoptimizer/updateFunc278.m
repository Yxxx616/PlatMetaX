% MATLAB Code
function [offspring] = updateFunc278(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    ...
    f_weights = 0.7 * (1 - (popfits - f_min) ./ (f_max - f_min)) + 0.3 * rand(NP, 1);
    c_weights = 0.7 * (1 - (abs_cons - c_min) ./ (c_max - c_min)) + 0.3 * rand(NP, 1);
