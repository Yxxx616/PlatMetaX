% MATLAB Code
function [offspring] = updateFunc281(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    ...
    f_weights = 0.5 * (1 + (popfits - f_min) ./ (f_max - f_min));
    c_weights = 0.5 * (1 + (abs_cons - c_min) ./ (c_max - c_min));
    F = 0.5 + 0.2 * randn(NP, 1);
    
    % Vectorized mutation operation with improved components
    ...
    offspring = popdecs + F.*d_elite + f_weights.*d_feas + ...
                c_weights.*d_cons + 0.3*d_rand;
    ...
    offspring = offspring.*(~out_low & ~out_high) + ...
               (0.5*reflect_low + 0.5*(lb + rand(NP,D).*(ub-lb))).*out_low + ...
               (0.5*reflect_high + 0.5*(lb + rand(NP,D).*(ub-lb))).*out_high;
    
    % Final boundary enforcement with smaller perturbation
    offspring = max(min(offspring, ub), lb) + 0.002 * randn(NP, D) .* (ub - lb);
