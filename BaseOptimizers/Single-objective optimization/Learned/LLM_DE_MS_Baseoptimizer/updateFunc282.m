% MATLAB Code
function [offspring] = updateFunc282(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    ...
    f_weights = 0.5 + 0.5*(popfits - f_min) ./ (f_max - f_min);
    c_weights = 0.5 + 0.5*(abs_cons - c_min) ./ (c_max - c_min);
    ...
    offspring = max(min(offspring, ub), lb) + 0.001 * randn(NP, D) .* (ub - lb);
