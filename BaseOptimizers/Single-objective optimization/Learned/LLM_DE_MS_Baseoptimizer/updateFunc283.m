% MATLAB Code
function [offspring] = updateFunc283(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    ...
    offspring = popdecs + F.*d_elite + f_weights.*d_feas + ...
                c_weights.*d_cons + 0.5*d_rand;
    
    % Boundary handling with perturbation
    out_low = offspring < lb;
    out_high = offspring > ub;
    
    offspring = offspring.*(~out_low & ~out_high) + ...
               (lb + 0.01*randn(NP,D).*(ub-lb)).*out_low + ...
               (ub - 0.01*randn(NP,D).*(ub-lb)).*out_high;
