% MATLAB Code
function [offspring] = updateFunc285(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    ...
    f_weights = 0.4 + 0.6*(popfits - f_min) ./ (f_max - f_min);
    c_weights = 0.4 + 0.6*(abs_cons - c_min) ./ (c_max - c_min);
    F = 0.5 + 0.2*sin(2*pi*rand(NP,1)); % Cyclic scaling factor
    ...
    offspring = popdecs + F.*d_elite + f_weights.*d_feas + ...
                c_weights.*d_cons + 0.7*d_rand;
    
    % Boundary handling with reflected bounce-back
    out_low = offspring < lb;
    out_high = offspring > ub;
    eta = 0.05 + 0.1*rand(NP,D); % Random perturbation factor
