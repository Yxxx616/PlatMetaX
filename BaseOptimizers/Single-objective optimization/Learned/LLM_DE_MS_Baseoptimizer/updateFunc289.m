% MATLAB Code
function [offspring] = updateFunc289(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    ...
    % Adaptive scaling factors based on rank
    F = 0.5 + 0.3*sin(pi*f_rank/NP);
    
    % Adaptive weights based on constraint violation
    sigma_c = 0.2 + 0.1*rand();
    exp_cons = exp(-norm_cons/sigma_c);
    w_total = exp_cons + exp(-norm_fits) + 0.1*rand(NP,1);
    
    w1 = (0.5*exp_cons + 0.3*exp(-norm_fits)) ./ w_total;
    w2 = (0.3*exp_cons + 0.4*exp(-norm_fits)) ./ w_total;
    w3 = 0.2*ones(NP,1) ./ w_total;
    w4 = (0.4*exp_cons + 0.1*exp(-norm_fits)) ./ w_total;
