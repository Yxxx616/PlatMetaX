% MATLAB Code
function [offspring] = updateFunc291(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    ...
    % Adaptive parameters
    sigma_c = 0.2 + 0.2*rand();
    sigma_f = 0.2 + 0.2*rand();
    ...
    w1 = (0.7*exp_cons + 0.1*exp_fits) ./ w_total;
    w2 = (0.2*exp_cons + 0.4*exp_fits) ./ w_total;
    w3 = 0.1*ones(NP,1) ./ w_total;
    w4 = (0.1*exp_cons + 0.5*exp_fits) ./ w_total;
