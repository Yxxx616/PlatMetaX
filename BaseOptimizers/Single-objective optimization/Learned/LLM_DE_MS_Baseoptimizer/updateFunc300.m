% MATLAB Code
function [offspring] = updateFunc300(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    ...
    % Adaptive parameters
    sigma_c = 0.2 + 0.3*rand();
    sigma_f = 0.2 + 0.3*rand();
    
    % Calculate adaptive weights
    exp_cons = exp(-norm_cons/sigma_c);
    exp_fits = exp(-norm_fits/sigma_f);
    w_total = 0.6*exp_cons + 0.6*exp_fits + 0.2 + 0.4*exp_cons + eps;
    
    w1 = (0.6*exp_cons + 0.4*exp_fits) ./ w_total;
    w2 = (0.4*exp_cons + 0.6*exp_fits) ./ w_total;
    w3 = 0.2 ./ w_total;
    w4 = 0.4*exp_cons ./ w_total;
    
    % Scaling factor with adaptation
    F = 0.5 + 0.3*rand(NP,1);
