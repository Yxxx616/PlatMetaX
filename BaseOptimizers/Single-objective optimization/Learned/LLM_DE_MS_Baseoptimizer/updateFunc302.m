% MATLAB Code
function [offspring] = updateFunc302(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    ...
    % Adaptive parameters
    sigma_c = 0.1 + 0.4*rand();
    sigma_f = 0.1 + 0.4*rand();
    ...
    % Weight components
    w1 = exp_cons;  % Elite direction weight
    w2 = exp_fits;  % Feasible direction weight
    w3 = ones(NP,1);  % Random direction weight
    w4 = 0.5*exp_cons + 0.5*exp_fits;  % Low-constraint direction weight
    
    % Normalize weights
    w_total = w1 + w2 + w3 + w4 + eps;
    w1 = w1 ./ w_total;
    w2 = w2 ./ w_total;
    w3 = w3 ./ w_total;
    w4 = w4 ./ w_total;
    
    % Adaptive scaling factor with fitness-based adjustment
    F_base = 0.4 + 0.4*rand(NP,1);
    F = F_base .* (1 - norm_fits).^2;
    ...
    % Improved adaptive boundary handling
    out_low = offspring < lb;
    out_high = offspring > ub;
    eta = 0.1 + 0.4*rand(NP,D);
