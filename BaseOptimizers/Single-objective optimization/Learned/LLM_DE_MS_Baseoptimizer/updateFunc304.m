% MATLAB Code
function [offspring] = updateFunc304(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    ...
    eta = 0.1 + 0.3*rand(NP,D);
    
    % Reflection with adaptive perturbation
    offspring = offspring.*(~out_low & ~out_high) + ...
               (lb + eta.*(ub - offspring)).*out_low + ...
               (ub - eta.*(offspring - lb)).*out_high;
    
    % Final boundary enforcement with small adaptive perturbation
    perturb = 0.02*(ub-lb).*randn(NP,D).*repmat(1-norm_fits,1,D);
