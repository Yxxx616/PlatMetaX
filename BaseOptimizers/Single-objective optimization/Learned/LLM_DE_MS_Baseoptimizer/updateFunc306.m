% MATLAB Code
function [offspring] = updateFunc306(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    ...
    F = 0.4 + 0.4*rand(NP,1) .* (1 - norm_fits).^0.5;
    ...
    perturb = 0.005*(ub-lb).*randn(NP,D).*repmat(1-norm_fits,1,D);
