% MATLAB Code
function [offspring] = updateFunc307(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    ...
    % Elite individual (balanced fitness and constraints)
    if any(feasible_mask)
        [~, elite_idx] = min(popfits(feasible_mask));
        elite = popdecs(feasible_mask,:);
        elite = elite(elite_idx,:);
    else
        combined_scores = 0.7*norm_fits + 0.3*norm_cons;
    ...
    % Calculate weights using modified scores
    w_elite = exp(-(alpha*norm_fits + (1-alpha)*norm_cons)/sigma);
    w_feas = exp(-norm_fits/sigma);
    w_rand = ones(NP,1);
    w_cons = exp(-norm_cons/sigma);
    ...
    % Adaptive scaling factor with stronger fitness adaptation
    ...
    % Final boundary enforcement with fitness-based perturbation
    perturb = 0.01*(ub-lb).*randn(NP,D).*repmat(1-norm_fits,1,D).^2;
