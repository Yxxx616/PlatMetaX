% MATLAB Code
function [offspring] = updateFunc308(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    ...
    % Combined score (adaptive balance)
    alpha = 0.6 + 0.2*rand();
    ...
    % Elite individual (balanced score)
    [~, elite_idx] = min(scores);
    elite = popdecs(elite_idx,:);
    
    % Best feasible individual
    if any(feasible_mask)
        [~, best_idx] = min(popfits(feasible_mask));
        best_feas = popdecs(feasible_mask(best_idx),:);
    else
        best_feas = elite;
    end
    ...
    F = 0.4 + 0.4*rand(NP,1).*(1-scores).^2;
    ...
    perturb = 0.005*(ub-lb).*randn(NP,D).*repmat(1-scores,1,D);
