% MATLAB Code
function [offspring] = updateFunc311(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    ...
    % Adaptive weights
    sigma_f = 0.3;  % Increased from 0.2 for better exploration
    sigma_c = 0.3;  % Increased from 0.2 for better constraint handling
    ...
    ranks = zeros(NP,1);
    ranks(rank_idx) = (1:NP)';
    F = 0.5 + 0.5 * erf((NP - ranks)/NP - 0.5);
    
    % Vectorized mutation with enhanced exploration
    ...
    % Boundary handling with reflection and small perturbation
    ...
    perturb = 0.05*(ub-lb).*randn(NP,D).*repmat(1-ranks/NP,1,D);
    offspring = max(min(offspring + perturb, ub), lb);
    
    % Ensure at least some exploration for top individuals
    explore_mask = ranks < 0.2*NP;
    if any(explore_mask)
        offspring(explore_mask,:) = offspring(explore_mask,:) + ...
            0.1*(ub-lb).*randn(sum(explore_mask),D);
