% MATLAB Code
function [offspring] = updateFunc316(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    ...
    offspring = offspring.*(~out_low & ~out_high) + ...
               (lb + mod(abs(offspring - lb), range)) .* out_low + ...
               (ub - mod(abs(offspring - ub), range)) .* out_high;
    
    % Adaptive local refinement
    perturb = 0.05*(ub-lb).*randn(NP,D).*repmat(1-ranks/NP,1,D);
    offspring = max(min(offspring + perturb, ub), lb);
    
    % Elite-guided exploration for top solutions
    explore_mask = ranks < max(3, 0.1*NP);
    if any(explore_mask)
        offspring(explore_mask,:) = offspring(explore_mask,:) + ...
            0.1*(ub-lb).*(rand(sum(explore_mask),D)-0.5) + ...
            0.05*(repmat(elite, sum(explore_mask), 1) - offspring(explore_mask,:));
