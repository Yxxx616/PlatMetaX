% MATLAB Code
function [offspring] = updateFunc324(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    ...
    % Adaptive weights with stronger focus
    sigma_f = 0.1;  % Strong fitness pressure
    sigma_c = 0.2;  % Moderate constraint pressure
    ...
    % Non-linear rank-based scaling factor (more aggressive)
    [~, rank_idx] = sort(popfits);
    ranks = zeros(NP,1);
    ranks(rank_idx) = (1:NP)';
    F = 0.1 + 0.7 * (1 - (ranks/NP).^0.3);  % More aggressive scaling
    ...
    % Enhanced exploration for worst 20% solutions
