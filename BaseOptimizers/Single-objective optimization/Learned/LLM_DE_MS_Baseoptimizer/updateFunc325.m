% MATLAB Code
function [offspring] = updateFunc325(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    ...
    sigma_f = 0.15;  % Fitness pressure
    sigma_c = 0.25;  % Constraint pressure
    ...
    ranks = zeros(NP,1);
    ranks(rank_idx) = (1:NP)';
    F = 0.2 + 0.6 * (1 - (ranks/NP).^0.4);  % Aggressive scaling
