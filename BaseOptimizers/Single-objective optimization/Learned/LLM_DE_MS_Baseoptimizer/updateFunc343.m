% MATLAB Code
function [offspring] = updateFunc343(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    ...
    ranks = zeros(NP,1);
    ranks(rank_idx) = (1:NP)';
    F = 0.3 + 0.5 * (1 - (ranks/NP).^0.5); % Modified exponent
    ...
    % Final boundary check with random component
    rand_comp = rand(NP,D).*(ub-lb) + lb;
    offspring = max(min(offspring, ub), lb).*0.9 + rand_comp.*0.1;
end