function [offspring] = updateFunc35(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    ...
    % Calculate fitness ranks (1 to NP)
    [~, fit_sort_idx] = sort(popfits);
    [~, fit_ranks] = sort(fit_sort_idx);
    norm_fits = fit_ranks / NP;  % Normalize to (0,1]
    
    % Calculate constraint violation ranks (1 to NP)
    abs_cons = abs(cons);
    [~, cons_sort_idx] = sort(abs_cons);
    [~, cons_ranks] = sort(cons_sort_idx);
    norm_cons = cons_ranks / NP;  % Normalize to (0,1]
    ...
    F_fit = 0.5 * (1 - norm_fits);
    F_cons = 0.5 * (1 - norm_cons);
