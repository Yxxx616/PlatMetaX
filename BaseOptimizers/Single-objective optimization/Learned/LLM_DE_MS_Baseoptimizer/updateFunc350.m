% MATLAB Code
function [offspring] = updateFunc350(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    ...
    % Randomization for boundary solutions
    boundary_mask = out_low | out_high;
    rand_mask = rand(NP,D) < 0.1; % Lower probability than before
    rand_vals = rand(NP,D).*(ub-lb) + lb;
    offspring = offspring.*~(boundary_mask & rand_mask) + ...
               rand_vals.*(boundary_mask & rand_mask);
end