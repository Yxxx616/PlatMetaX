% MATLAB Code
function [offspring] = updateFunc357(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    ...
    offspring = popdecs + F.*base_mutation + ...
                0.05*(1-F).*randn(NP,D).*(ub-lb);
    ...
    % Additional perturbation for infeasible solutions
    perturb_mask = ~feasible_mask;
    if any(perturb_mask)
        offspring(perturb_mask,:) = offspring(perturb_mask,:) + ...
            0.1*randn(sum(perturb_mask),D).*(ub-lb);
    end
end