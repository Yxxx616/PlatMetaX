% MATLAB Code
function [offspring] = updateFunc358(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    ...
    % Enhanced perturbation for infeasible solutions
    perturb_mask = ~feasible_mask;
    if any(perturb_mask)
        offspring(perturb_mask,:) = offspring(perturb_mask,:) + ...
            0.15*randn(sum(perturb_mask),D).*(ub-lb);
    end
end