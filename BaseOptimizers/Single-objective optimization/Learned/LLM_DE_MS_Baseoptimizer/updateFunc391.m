% MATLAB Code
function [offspring] = updateFunc391(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    we = 0.7 * (1 - rho) .* (1 - norm_cons);
    wx = 0.5 * rho .* (1 - norm_fits);
    wd = 0.3 * rand(NP, 1);
    ...
    offspring = popdecs + we .* diff_feas + wx .* diff_best + wd .* diff_div;
    ...
    std_pop = std(popdecs);
    perturb = 0.1 * (ub - lb) .* repmat(std_pop, NP, 1) .* randn(NP, D);
