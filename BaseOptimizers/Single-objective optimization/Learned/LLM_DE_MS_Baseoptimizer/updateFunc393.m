% MATLAB Code
function [offspring] = updateFunc393(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    F1 = 0.4 * (1 - rho) .* (1 - norm_cons);
    F2 = 0.4 * rho .* (1 - norm_fits);
    F3 = 0.2 * rand(NP, 1);
    ...
    % Adaptive perturbation based on population diversity and feasibility
    std_pop = std(popdecs);
    sigma = 0.1 * (1 - rho) * (ub - lb);
    perturb = repmat(sigma, NP, 1) .* repmat(std_pop, NP, 1) .* randn(NP, D);
