% MATLAB Code
function [offspring] = updateFunc394(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    dist_to_feas = sqrt(sum((popdecs - best_feas).^2, 2));
    sigma = 0.1 * (1 - rho) * dist_to_feas;
    offspring = offspring + repmat(sigma, 1, D) .* randn(NP, D);
