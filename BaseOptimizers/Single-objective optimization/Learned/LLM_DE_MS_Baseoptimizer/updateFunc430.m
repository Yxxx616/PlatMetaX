% MATLAB Code
function [offspring] = updateFunc430(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    eps = 1e-12;
    ...
    % Adaptive weights
    F1 = 0.8 * rho * (1 - norm_fits);
    F2 = 0.9 * (1 - rho) * (1 - norm_cons);
    F3 = 0.5;
    ...
    % Combined mutation
    offspring = popdecs + F1.*best_dir + F2.*feas_dir + F3.*div_dir + perturb;
