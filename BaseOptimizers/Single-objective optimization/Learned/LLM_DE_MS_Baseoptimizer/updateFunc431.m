% MATLAB Code
function [offspring] = updateFunc431(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    eps = 1e-12;
    ...
    % Combined mutation with adaptive perturbation
    sigma = 0.1 + 0.2 * (1 - rho);
    perturb = sigma * randn(NP, D);
    offspring = popdecs + F1.*feas_dir + F2.*best_dir + F3.*div_dir + perturb;
