function [offspring] = updateFunc45(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    ...
    for i = 1:NP
        ...
        delta_fit = ((popfits(r2) - popfits(r1)) / fit_denom) .* ...
                   (popdecs(r2,:) - popdecs(r1,:));
        
        % Adaptive scaling factor
        F = 0.5 + 0.5 * (popfits(r3) - f_min) / (f_max - f_min + eps);
        
        % Constraint-aware perturbation
        cons_weight = eta * tanh(abs(cons(r3)));
        delta_cons = cons_weight .* (popdecs(r4,:) - popdecs(r3,:));
        
        % Diversity component
        delta_div = alpha * randn(1,D) .* (ub - lb);
        
        % Mutation
        offspring(i,:) = popdecs(r3,:) + F * delta_fit + delta_cons + delta_div;
    end
