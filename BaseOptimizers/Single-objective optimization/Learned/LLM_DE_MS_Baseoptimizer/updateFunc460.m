% MATLAB Code
function [offspring] = updateFunc460(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    eps = 1e-12;
    ...
    % Normalize fitness and constraints [0,1]
    norm_fits = (popfits - min(popfits)) / (max(popfits) - min(popfits) + eps);
    norm_cons = (cons - min(cons)) / (max(cons) - min(cons) + eps);
    
    % Adaptive scaling factors
    F1 = 0.6 * alpha * (1 - norm_fits);
    F2 = 0.4 * (1 - alpha) * (1 - norm_cons);
    F3 = 0.2 * alpha * ones(NP, 1);
    F4 = 0.2 * (1 - alpha) * ones(NP, 1);
    ...
    % Generate random indices for differential component
    r1 = randi(NP, NP, 1);
    r2 = randi(NP, NP, 1);
