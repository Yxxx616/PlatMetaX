% MATLAB Code
function [offspring] = updateFunc466(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    eps = 1e-12;
    ...
    F_elite = 0.4 * (1 - alpha) * (0.8 + 0.4*rand(NP, 1));
    F_center = 0.3 * alpha * (0.7 + 0.3*rand(NP, 1));
    F_con = 0.5 * (1 - norm_cons).^3;  % Stronger focus on feasible solutions
    F_rand = 0.2 * norm_cons.^2;
    ...
    reflect_prob = 0.6 + 0.3*norm_cons;  % More reflection for infeasible
