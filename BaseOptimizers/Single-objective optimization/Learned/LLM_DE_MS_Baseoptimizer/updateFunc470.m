% MATLAB Code
function [offspring] = updateFunc470(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    eps = 1e-12;
    ...
    F_elite = 0.4 * (1 - alpha) * (0.3 + 0.7*randn(NP, 1));
    F_center = 0.3 * alpha * (0.4 + 0.6*rand(NP, 1));
    F_con = 0.5 * (1 - norm_cons).^3;
    F_rand = 0.2 * norm_cons.^1.2;
