% MATLAB Code
function [offspring] = updateFunc473(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    eps = 1e-12;
    ...
    % Normalized constraints with exponential transformation
    norm_cons = (cons - min(cons)) / (max(cons) - min(cons) + eps);
    
    % Adaptive weights
    w_elite = 0.5*(1-alpha) + 0.2*randn(NP, 1);
    w_con = 0.3 * exp(-2 * norm_cons);
    w_rand = 0.2 * norm_cons.^1.5;
