% MATLAB Code
function [offspring] = updateFunc481(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    eps = 1e-12;
    ...
    % Adaptive weights
    w_elite = 0.5*alpha + 0.1*rand(NP, 1);
    ...
    offspring = offspring .* ~(mask_low | mask_high) + ...
               (lb_rep + (ub_rep-lb_rep).*rand(NP,D)) .* (mask_low | mask_high);
