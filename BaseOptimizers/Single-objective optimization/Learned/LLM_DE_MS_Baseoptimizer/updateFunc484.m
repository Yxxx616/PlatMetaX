% MATLAB Code
function [offspring] = updateFunc484(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    eps = 1e-12;
    ...
    % Adaptive weights
    w_elite = 0.6*alpha + 0.2*(1-alpha)*rand(NP, 1);
    w_con = 0.5 * exp(-3 * norm_cons);
    w_fit = 0.4 * (1 - norm_fits);
    sigma = 0.3 * (1-alpha);
