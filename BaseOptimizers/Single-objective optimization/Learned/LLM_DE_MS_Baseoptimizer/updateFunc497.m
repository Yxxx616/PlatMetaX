% MATLAB Code
function [offspring] = updateFunc497(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    F1 = 0.7;  % Strong elite guidance
    F2 = 0.3 + 0.4 * rand(NP,1);  % Fitness-directed exploration
    F3 = 0.1 + 0.2 * rand(NP,1);  % Constraint-aware correction
    F4 = 0.2;  % Adaptive perturbation
