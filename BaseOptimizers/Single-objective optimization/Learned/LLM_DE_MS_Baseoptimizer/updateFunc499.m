% MATLAB Code
function [offspring] = updateFunc499(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    F1 = 0.9;  % Strong elite guidance
    F2 = 0.1 + 0.2 * rand(NP,1);  % Fitness-directed exploration
    F3 = 0.05 + 0.1 * rand(NP,1);  % Constraint-aware correction
    F4 = 0.05;  % Adaptive perturbation
