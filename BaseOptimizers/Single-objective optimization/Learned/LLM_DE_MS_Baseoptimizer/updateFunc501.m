% MATLAB Code
function [offspring] = updateFunc501(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    % Adaptive scaling factors (modified from previous version)
    F1 = 0.7 + 0.2 * rand(NP,1);  % Strong elite guidance
    F2 = 0.4 + 0.3 * rand(NP,1);  % Fitness-directed exploration
    F3 = 0.2 + 0.2 * rand(NP,1);  % Constraint-aware correction
    ...
    rand_pert = randn(NP,D) .* repmat((ub-lb)/20, NP, 1);  % Reduced perturbation
