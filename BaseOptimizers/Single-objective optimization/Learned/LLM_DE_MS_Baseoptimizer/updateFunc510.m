% MATLAB Code
function [offspring] = updateFunc510(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    eps = 1e-10;
    ...
    min_fit = min(popfits);
    max_fit = max(popfits);
    w_f = ((popfits - min_fit) / (max_fit - min_fit + eps)).^0.7;
    
    abs_cons = abs(cons);
    max_con = max(abs_cons);
    w_c = (abs_cons / (max_con + eps)).^1.3;
    ...
    F3 = 0.3 * (0.2 + 0.8*w_c) .* rand(NP,1);
    
    % Adaptive random perturbation
    rand_pert = 0.04 * randn(NP,D) .* (1 - sqrt(w_f.*w_c)) .* (ub - lb);
    ...
    reflect_factor = 0.4 + 0.6*repmat(sqrt(w_f.*w_c),1,D);
