% MATLAB Code
function [offspring] = updateFunc519(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    eps = 1e-12;
    ...
    w = (1 - norm_fit).^1.5 .* (1 + norm_con).^0.4;
    
    % Elite selection with constraint priority
    feasible = cons <= 0;
    if any(feasible)
        ...
        [~, elite_idx] = min(norm_fit + 0.7*norm_con);
        elite = popdecs(elite_idx,:);
    end
    
    % Random indices with improved diversity
    idx = randperm(NP);
    r1 = mod(idx, NP) + 1;
