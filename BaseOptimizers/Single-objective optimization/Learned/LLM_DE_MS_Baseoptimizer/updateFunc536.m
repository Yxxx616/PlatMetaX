% MATLAB Code
function [offspring] = updateFunc536(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    eps = 1e-12;
    ...
    F1 = 0.5 * ones(NP, 1);  % Elite guidance strength
    F2 = 0.7 * (1 - norm_fit);  % Fitness-based exploration
    F3 = 0.3 * norm_con;  % Constraint-driven perturbation
