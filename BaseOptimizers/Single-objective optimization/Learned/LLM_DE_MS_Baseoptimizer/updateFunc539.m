% MATLAB Code
function [offspring] = updateFunc539(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    eps = 1e-12;
    ...
    F1 = 0.7 * (1 - w);          % Elite guidance strength
    F2 = 0.4 * w + 0.2 * v;      % Constraint-aware perturbation
    F3 = 0.1 * (1 - v);          % Exploration control
