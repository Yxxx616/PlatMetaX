% MATLAB Code
function [offspring] = updateFunc554(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    perturbation = randn(NP, D) .* repmat(sigma, 1, D) .* repmat(1 - w_f, 1, D);
    
    % Combined mutation
    offspring = popdecs + ...
        repmat(F_e, 1, D) .* elite_diff + ...
        repmat(F_c, 1, D) .* rand_diff .* repmat(1 + w_c, 1, D) + ...
        perturbation;
