% MATLAB Code
function [offspring] = updateFunc567(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    % Select elite considering both fitness and constraints
    ...
    if any(feasible_mask)
        ...
        [~, elite_idx] = min(popfits + 1e6*norm_cons); % Penalize infeasible
