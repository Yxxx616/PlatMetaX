% MATLAB Code
function [offspring] = updateFunc580(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    weights = 1 - (ranks-1)/(NP-1);  % Weight [1,0] best to worst
    ...
    % Adaptive parameters
    F = 0.4 + 0.4 * weights .* (1 - cons_norm);
    
    % Directional mutation
    elite_diff = repmat(elite, NP, 1) - popdecs;
    rand_diff = popdecs(r1,:) - popdecs(r2,:);
    weight_factors = 1 - weights;
    mutant = popdecs + F .* elite_diff + F .* rand_diff .* weight_factors;
    
    % Adaptive crossover
    CR = 0.1 + 0.7 * weights .* (1 - cons_norm);
    j_rand = randi(D, NP, 1);
