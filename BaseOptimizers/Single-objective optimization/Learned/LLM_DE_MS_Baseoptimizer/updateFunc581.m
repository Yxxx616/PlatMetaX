% MATLAB Code
function [offspring] = updateFunc581(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    if any(feasible)
        ...
        [~, elite_idx] = min(popfits + 10*cons_pos); % Strong penalty
        elite = popdecs(elite_idx, :);
    end
    
    % Rank population (1=best, NP=worst)
    [~, rank_order] = sort(popfits + 10*cons_pos); % Combined ranking
    ranks = zeros(NP,1);
    ranks(rank_order) = 1:NP;
    rank_weights = 1 - (ranks-1)/(NP-1);  % Weight [1,0] best to worst
    ...
    F = 0.5 * (1 + rank_weights) .* (1 - cons_norm);
    ...
    % Adaptive crossover
    CR = 0.3 + 0.5 * rank_weights .* (1 - cons_norm);
    j_rand = randi(D, NP, 1);
