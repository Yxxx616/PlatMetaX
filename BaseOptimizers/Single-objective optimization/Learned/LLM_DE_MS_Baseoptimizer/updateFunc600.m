% MATLAB Code
function [offspring] = updateFunc600(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    % Adaptive parameters
    F = 0.3 + 0.5 * (1 - cons_norm) + 0.2 * f_norm;
    CR = 0.1 + 0.6 * (1 - cons_norm) + 0.3 * f_norm;
    ...
    % Adaptive perturbation for infeasible solutions
    perturb = 0.1 * (ub - lb) .* randn(NP, D);
    perturb_scale = cons_norm ./ max(cons_norm);
    offspring = offspring + perturb .* perturb_scale(:, ones(1,D));
    offspring = min(max(offspring, lb_rep), ub_rep);
end