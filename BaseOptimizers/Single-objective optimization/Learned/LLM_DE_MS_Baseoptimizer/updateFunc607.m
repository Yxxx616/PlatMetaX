% MATLAB Code
function [offspring] = updateFunc607(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    high_constraint = w_c > 0.9;
    if any(high_constraint)
        idx = find(high_constraint);
        sigma = 0.1 + 0.2 * w_c(idx);
        mutant(idx,:) = mutant(idx,:) + sigma .* randn(length(idx), D) .* (ub - lb);
    end
    
    % Adaptive crossover with fitness and constraint awareness
    CR = 0.2 + 0.6 * (1 - cons_norm) .* (1 - f_norm);
    j_rand = randi(D, NP, 1);
