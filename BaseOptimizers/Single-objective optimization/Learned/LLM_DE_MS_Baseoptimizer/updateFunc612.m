% MATLAB Code
function [offspring] = updateFunc612(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    % Adaptive parameters
    base_F = 0.5 + 0.3 * rand(NP, 1) .* (1 - f_norm);
    F = min(max(base_F, 0.3), 0.9);
    w_f = 0.6 * (1 - f_norm);
    w_c = 0.4 * cons_norm;
    ...
    % Adaptive crossover
    CR_base = 0.6 + 0.2 * (1 - f_norm) .* (1 - cons_norm);
    CR = min(max(CR_base, 0.1), 0.9);
    j_rand = randi(D, NP, 1);
