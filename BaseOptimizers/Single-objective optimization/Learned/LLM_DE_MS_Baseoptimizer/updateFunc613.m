% MATLAB Code
function [offspring] = updateFunc613(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    
    % Normalize constraints (handle all feasible case)
    cons_pos = max(0, cons);
    if max(cons_pos) > 0
        cons_norm = cons_pos / max(cons_pos);
    else
        cons_norm = zeros(NP, 1);
    end
    ...
    % Adaptive crossover
    CR_base = 0.7 + 0.2 * (1 - f_norm) .* (1 - cons_norm);
    CR = min(max(CR_base, 0.3), 0.95);
    j_rand = randi(D, NP, 1);
