function [offspring] = updateFunc62(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    ...
    for i = 1:NP
        ...
        F2 = 0.5 * (1 - cons(i) / (phi_max + eps));
        
        % Mutation based on constraint violation
        if cons(i) <= phi_avg
        ...
        % Boundary control with bounce-back
        for j = 1:D
            if offspring(i,j) < -500
                offspring(i,j) = -500 + rand*(x_best(j) - (-500));
            elseif offspring(i,j) > 500
                offspring(i,j) = 500 - rand*(500 - x_best(j));
            end
        end
    end
end