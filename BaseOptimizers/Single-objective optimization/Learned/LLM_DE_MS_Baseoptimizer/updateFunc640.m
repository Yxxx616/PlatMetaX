% MATLAB Code
function [offspring] = updateFunc640(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    F = 0.5 * (1 + tanh((mean_cons - cons)./(std_cons + eps)));
    p_elite = 0.2 + 0.3 * (1 - elite_fit/(max_fit + eps));
    ...
    for i = 1:NP
        ...
        scale = 1 - cons(i)/(max(cons)+eps);
        v2 = popdecs(idx(1),:) + F(i)*(popdecs(idx(2),:) - popdecs(idx(3),:))*scale;
        
        % Random exploration
        v3 = popdecs(idx(4),:) + F(i)*(popdecs(idx(5),:) - popdecs(idx(6),:));
