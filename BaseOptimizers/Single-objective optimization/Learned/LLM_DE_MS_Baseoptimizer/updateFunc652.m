% MATLAB Code
function [offspring] = updateFunc652(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    
    % 1. Elite selection considering both fitness and constraints
    feasible = cons <= 0;
    if any(feasible)
    ...
    F = 0.4 + 0.3 * (cons - min_cons) ./ (max_cons - min_cons + eps) + ...
