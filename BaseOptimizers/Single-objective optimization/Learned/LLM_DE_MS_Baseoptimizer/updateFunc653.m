% MATLAB Code
function [offspring] = updateFunc653(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    min_cv = min(cons);
    max_cv = max(cons);
    F = 0.3 + 0.5*(ranks/NP).^0.7 + 0.2*(cons-min_cv)/(max_cv-min_cv+eps);
    CR = 0.1 + 0.8*(1-ranks/NP).^1.5;
    
    % 3. Generate random indices matrix
    idx_mat = zeros(NP, 4);
    for i = 1:NP
    ...
    for i = 1:NP
        ...
        if cons(i) <= 0
            % Feasible strategy
            mutant(i,:) = elite + F(i)*(popdecs(r1,:)-popdecs(r2,:)) + ...
                          F(i)*(popdecs(r3,:)-popdecs(r4,:));
        else
            % Infeasible strategy
            mutant(i,:) = popdecs(i,:) + F(i)*(elite-popdecs(i,:)) + ...
    ...
    offspring(still_invalid) = lb_rep(still_invalid) + ...
                              rand(sum(still_invalid(:)),1).* ...
                              (ub_rep(still_invalid)-lb_rep(still_invalid));
end