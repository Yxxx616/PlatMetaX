% MATLAB Code
function [offspring] = updateFunc657(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    F_base = 0.5 * (1 - ranks/NP).^1.5;
    F_feas = 0.3 * exp(-cons./(sigma_cv + eps));
    F_divers = 0.2 * (1 - ranks/NP).^0.8;
    ...
    mutant = popdecs + ...
        ...
        F_feas.*(repmat(feas_best, NP, 1) - popdecs) + ...
        F_divers.*(popdecs(r1,:) - popdecs(r2,:) + popdecs(r3,:) - popdecs(r4,:));
    
    % 6. Crossover with adaptive CR
    CR = 0.1 + 0.8*(1 - ranks/NP).^2.0;
