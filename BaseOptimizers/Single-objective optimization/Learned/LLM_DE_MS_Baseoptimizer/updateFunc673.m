% MATLAB Code
function [offspring] = updateFunc673(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    % Random perturbation
    rand_dir = randn(NP, D) .* (ub-lb)/10;
    
    % 6. Adaptive scaling factors
    F_elite = 0.5 + 0.3 * randn(NP,1);
    F_rank = 0.4 * (1 - norm_ranks);
    F_cons = 0.3 * (1 - norm_cons);
    F_rand = 0.1 * (1 - norm_ranks);
    
    % 7. Mutation operation
    mutant = popdecs + F_elite.*elite_dir + F_rank.*rank_dir + ...
             F_cons.*cons_dir + F_rand.*rand_dir;
    
    % 8. Improved crossover
    CR = 0.8 + 0.1 * (1 - norm_ranks);
    mask = rand(NP, D) < CR;
    j_rand = randi(D, NP, 1);
