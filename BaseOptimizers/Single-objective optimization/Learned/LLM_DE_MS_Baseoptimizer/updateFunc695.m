% MATLAB Code
function [offspring] = updateFunc695(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    eps = 1e-12;
    ...
    F = 0.4 + 0.3 * norm_f + 0.3 * cons_ratio;
    
    % 2. Select elite and best individuals
    ...
    cons_perturb = bsxfun(@times, popdecs(rand_idx1,:) - popdecs(rand_idx2,:), sign(cons));
    fit_diff = bsxfun(@minus, best, popdecs) + (popdecs(rand_idx3,:) - popdecs(rand_idx4,:));
    
    % 5. Hybrid mutation
    mutant = popdecs + bsxfun(@times, elite_dir + 0.5*cons_perturb + 0.5*fit_diff, F);
    
    % 6. Rank-based adaptive crossover
    [~, rank_order] = sort(penalty);
    ranks = zeros(NP,1);
    ranks(rank_order) = 1:NP;
    CR = 0.9 * (1 - ranks/NP).^0.3;
