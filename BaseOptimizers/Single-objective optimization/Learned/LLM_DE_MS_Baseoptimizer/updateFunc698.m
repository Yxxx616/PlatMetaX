% MATLAB Code
function [offspring] = updateFunc698(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    eps = 1e-12;
    ...
    F_base = 0.5 + 0.3 * norm_f;
    F_cons = 0.3 + 0.5 * cons_ratio;
    ...
    elite_dir = bsxfun(@times, elite - popdecs, F_base);
    
    cons_sign = sign(cons);
    cons_dir = bsxfun(@times, popdecs(rand_idx1,:) - popdecs(rand_idx2,:), ...
               cons_sign(:, ones(1,D))) .* F_cons(:, ones(1,D));
    
    fit_dir = bsxfun(@times, best - popdecs, (1-F_base)) + ...
