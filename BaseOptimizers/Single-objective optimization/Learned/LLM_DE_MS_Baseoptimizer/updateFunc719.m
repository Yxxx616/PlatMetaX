% MATLAB Code
function [offspring] = updateFunc719(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    eps = 1e-12;
    ...
    % Adaptive scaling factor with sine modulation
    F_base = 0.4 + 0.3 * sin(pi * iter / (2 * max_iter));
    F = F_base + 0.1 * w_f;
