% MATLAB Code
function [offspring] = updateFunc722(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    eps = 1e-12;
    ...
    % Adaptive scaling factor with cosine modulation and constraint guidance
    F_base = 0.5 * (1 + cos(pi * iter / (2 * max_iter)));
    F = F_base .* (1 - sqrt(w_c));
    ...
    % Combined mutation with adaptive components
    mutant = base + F .* (diff1 + diff_f + diff_c);
    
    % Dynamic crossover rate
    CR = 0.85 * (1 - sqrt(w_f .* w_c));
    ...
    % Adaptive reflection based on constraints
    reflect_factor = 0.3 + 0.7*w_c(:, ones(1, D));
    ...
    rand_perturb = 0.02 * (rand(NP, D) .* (ub_rep - lb_rep));
