% MATLAB Code
function [offspring] = updateFunc724(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    eps = 1e-12;
    ...
    mutant = base + F_f .* diff_f + F_c .* diff_c + (F_f + F_c) .* diff_mix;
    
    % Dynamic crossover rate
    CR = 0.9 * (1 - sqrt(F_f .* F_c));
    ...
    offspring = offspring + 0.01*(rand(NP,D)-0.5).*(ub_rep-lb_rep);
    offspring = max(min(offspring, ub_rep), lb_rep);
end