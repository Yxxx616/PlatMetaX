% MATLAB Code
function [offspring] = updateFunc727(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    eps = 1e-12;
    ...
    % Adaptive scaling factors (inverse relationship with fitness and constraints)
    F = 0.4 + 0.4 * (1 - w_c) .* (1 - w_f);
    F = repmat(F, 1, D);
    ...
    CR = 0.8 - 0.3 * sqrt(w_f .* w_c);
    CR = max(min(CR, 0.95), 0.1);
    ...
    % Adaptive boundary handling with small perturbation
