% MATLAB Code
function [offspring] = updateFunc730(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    eps = 1e-12;
    ...
    mutant = base + F .* direction;
    
    % Exponential crossover rate
    CR = 0.9 ./ (1 + exp(5*(w_f - 0.5)));
    CR = repmat(CR, 1, D);
    
    % Exponential crossover with guaranteed change
    mask = rand(NP, D) < CR;
    j_rand = randi(D, NP, 1);
