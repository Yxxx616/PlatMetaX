% MATLAB Code
function [offspring] = updateFunc742(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    eps = 1e-12;
    ...
    % Generate random indices for diversity and constraint components
    r1 = randi(NP, NP, 1);
    r2 = randi(NP, NP, 1);
    while any(r1 == r2)
    ...
    v_cons = sign(cons) .* (popdecs(r3,:) - popdecs(r4,:));
    
    % Adaptive weights
    F = 0.8;
    alpha = 0.4 * (1 - repmat(w_f, 1, D));
    beta = 0.3 * (1 - repmat(w_c, 1, D));
    gamma = 0.2 * repmat(w_f, 1, D);
    delta = 0.1 * repmat(w_c, 1, D);
    
    % Combined mutation
    mutant = popdecs + F .* (alpha .* v_elite + beta .* v_feas + gamma .* v_div + delta .* v_cons);
