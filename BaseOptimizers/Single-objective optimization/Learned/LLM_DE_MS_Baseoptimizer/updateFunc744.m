% MATLAB Code
function [offspring] = updateFunc744(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    eps = 1e-12;
    ...
    v_cons = sign(cons) .* (popdecs(r1,:) - popdecs(r2,:));
    
    % Adaptive weights and scale factor
    alpha = 0.5 * (1 - repmat(w_f, 1, D));
    beta = 0.3 * (1 - repmat(w_c, 1, D));
    gamma = 0.2 * repmat(w_c, 1, D);
    F = 0.5 + 0.3 * rand(NP, 1);
    
    % Combined mutation
    mutant = popdecs + F .* (alpha .* v_elite + beta .* v_feas + gamma .* v_cons);
