% MATLAB Code
function [offspring] = updateFunc749(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    eps = 1e-12;
    
    % Identify elite, best feasible and constraint-based solutions
    feasible = cons <= 0;
    if any(feasible)
    ...
    % Identify low and high constraint violation solutions
    ...
    % Adaptive weights
    alpha = 0.4 * (1 - repmat(w_f, 1, D)) + 0.1 * rand(NP, D);
    beta = 0.3 * (1 - repmat(w_c, 1, D)) + 0.1 * rand(NP, D);
    gamma = 0.2 * rand(NP, D);
    delta = 0.1 * repmat(w_c, 1, D) + 0.05 * rand(NP, D);
    
    % Constraint-adaptive scale factor
    F = 0.6 + 0.3 * tanh(3 * repmat(w_c, 1, D));
    
    % Combined mutation
    mutant = popdecs + F .* (alpha .* v_elite + beta .* v_feas + gamma .* v_rand + delta .* v_cons);
    
    % Constraint-adaptive crossover
    CR = 0.85 - 0.4 * repmat(w_c, 1, D);
    mask = rand(NP, D) < CR;
    j_rand = randi(D, NP, 1);
    ...
    % Boundary handling with reflection and projection
