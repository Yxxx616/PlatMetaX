% MATLAB Code
function [offspring] = updateFunc755(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    eps = 1e-12;
    ...
    beta = 0.3 * repmat(w_c, 1, D) + 0.2 * rand(NP, D);
    gamma = 0.2 * rand(NP, D);
    delta = 0.4 * (1 - repmat(w_c, 1, D)) + 0.2 * rand(NP, D);
    
    % Constraint-adaptive scale factor
    F = 0.4 + 0.4 * (1 - tanh(5 * repmat(w_c, 1, D)));
