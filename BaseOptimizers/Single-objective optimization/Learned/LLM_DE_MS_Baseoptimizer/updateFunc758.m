% MATLAB Code
function [offspring] = updateFunc758(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    eps = 1e-12;
    ...
    r2 = randi(NP, NP, 1);
    mask = (r1 == (1:NP)');
    r1(mask) = mod(r1(mask) + randi(NP-1, sum(mask), NP) + 1;
    mask = (r2 == (1:NP)');
    ...
    % Adaptive weights
    alpha = 0.5 * (1 - repmat(w_f, 1, D)) + 0.2 * rand(NP, D);
    beta = 0.4 * repmat(w_c, 1, D) + 0.2 * rand(NP, D);
    gamma = 0.3 * rand(NP, D);
    delta = 0.5 * (1 - repmat(w_c, 1, D)) + 0.2 * rand(NP, D);
    
    % Constraint-adaptive scale factor
    F = 0.6 + 0.3 * (1 - tanh(5 * repmat(w_c, 1, D)));
