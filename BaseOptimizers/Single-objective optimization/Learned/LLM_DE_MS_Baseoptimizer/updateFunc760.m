% MATLAB Code
function [offspring] = updateFunc760(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    eps = 1e-12;
    ...
    % Adaptive weights
    alpha = 0.4 * (1 - repmat(w_f, 1, D)) + 0.2 * rand(NP, D);
    beta = 0.3 * (1 - repmat(w_c, 1, D)) + 0.2 * rand(NP, D);
    gamma = 0.3 * repmat(w_c, 1, D) + 0.2 * rand(NP, D);
    delta = 0.3 * rand(NP, D);
    
    % Constraint-adaptive scale factor
    F = 0.5 + 0.3 * (1 - tanh(4 * repmat(w_c, 1, D)));
