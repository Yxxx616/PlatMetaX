% MATLAB Code
function [offspring] = updateFunc764(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    eps = 1e-12;
    ...
    r1 = arrayfun(@(i) setdiff(randperm(NP), i), 1:NP, 'UniformOutput', false);
    r1 = cell2mat(r1)';
    r1 = r1(:,1);
    r2 = arrayfun(@(i) setdiff(randperm(NP), [i, r1(i)]), 1:NP, 'UniformOutput', false);
    r2 = cell2mat(r2)';
    r2 = r2(:,1);
    ...
    % Adaptive weights with emphasis on constraints
    ...
    F = 0.5 + 0.3 * (1 - tanh(2 * repmat(w_c, 1, D)));
