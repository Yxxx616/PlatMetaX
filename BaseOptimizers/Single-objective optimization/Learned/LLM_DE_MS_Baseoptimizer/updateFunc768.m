% MATLAB Code
function [offspring] = updateFunc768(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    eps = 1e-12;
    ...
    % Adaptive weights
    alpha = 0.7 * (1 - repmat(w_f, 1, D)) + 0.1 * rand(NP, D);
    beta = 0.3 * repmat(w_c, 1, D) + 0.1 * rand(NP, D);
    gamma = 0.2 * (1 - repmat(w_c, 1, D)) + 0.1 * rand(NP, D);
    
    % Combined mutation vector
    v = alpha .* v_elite + beta .* v_rand + gamma .* v_cons;
