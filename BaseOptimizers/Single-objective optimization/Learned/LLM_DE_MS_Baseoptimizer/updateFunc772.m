% MATLAB Code
function [offspring] = updateFunc772(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    eps = 1e-12;
    ...
    % Identify extreme solutions for constraints and fitness
    ...
    beta = 0.3 * repmat(w_c, 1, D) + 0.1 * rand(NP, D);
    gamma = 0.3 * (1 - repmat(w_c, 1, D)) + 0.1 * rand(NP, D);
    delta = 0.3 * repmat(w_f, 1, D) + 0.1 * rand(NP, D);
    
    % Combined mutation vector
    v = alpha .* v_elite + beta .* v_cons + gamma .* v_fit + delta .* v_rand;
    
    % Dynamic scaling factor
    F = 0.5 + 0.3 * tanh(4 * repmat(w_c, 1, D));
