% MATLAB Code
function [offspring] = updateFunc789(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    eps = 1e-12;
    ...
    % Adaptive weights with stronger emphasis on elite and constraints
    alpha = 0.6 * (1 - w_f) + 0.2 * rand(NP, 1);
    beta = 0.4 * w_f + 0.2 * rand(NP, 1);
    gamma = 0.5 * w_c + 0.2 * rand(NP, 1);
    delta = 0.4 * (1 - w_c) + 0.2 * rand(NP, 1);
    
    % Combine components with weighted sum
    v = alpha .* v_elite + beta .* v_fit + gamma .* v_cons + delta .* v_rand;
    
    % Adaptive scaling factor with wider range
    F = 0.6 + 0.4 * (1 - w_c);
    
    % Mutation with vectorized operations
    mutant = popdecs + F .* v;
    
    % Adaptive crossover with wider range
    CR = 0.9 - 0.6 * w_f;
