% MATLAB Code
function [offspring] = updateFunc792(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    eps = 1e-12;
    ...
    % Adaptive weights
    alpha = 0.7 * (1 - w_f) + 0.3 * rand(NP, 1);
    beta = 0.5 * w_f + 0.3 * rand(NP, 1);
    gamma = 0.8 * w_c + 0.2 * rand(NP, 1);
    delta = 0.4 * (1 - w_c) + 0.3 * rand(NP, 1);
