% MATLAB Code
function [offspring] = updateFunc813(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    eps = 1e-12;
    ...
    % Adaptive weights with more exploration
    alpha = 0.5 * (1 - w_f) + 0.1 * rand(NP, 1);
    beta = 0.3 * w_c + 0.1 * rand(NP, 1);
    gamma = 0.2 + 0.1 * rand(NP, 1);
    
    % Combined mutation vector
    v = alpha .* v_fit + beta .* v_cons + gamma .* v_elite;
    F = 0.6 - 0.4 * w_c;
    mutant = popdecs + F .* v;
    
    % More aggressive crossover for better solutions
    CR = 0.9 - 0.5 * w_f;
