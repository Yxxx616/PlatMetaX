% MATLAB Code
function [offspring] = updateFunc816(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    eps = 1e-12;
    ...
    % Adaptive weights with balanced exploration/exploitation
    alpha = 0.6 * (1 - w_f) + 0.2 * rand(NP, 1);
    beta = 0.4 * w_c + 0.1 * rand(NP, 1);
    gamma = 0.3 + 0.2 * rand(NP, 1);
