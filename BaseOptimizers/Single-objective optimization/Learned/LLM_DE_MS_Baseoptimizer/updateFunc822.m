% MATLAB Code
function [offspring] = updateFunc822(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    eps = 1e-12;
    ...
    % Adaptive weights using tanh and sin for smooth transitions
    alpha = 0.5 * (1 - tanh(w_c));
    beta = 0.3 * tanh(w_c);
    ...
    F = 0.5 + 0.3 * (1 - w_c);
    v = alpha .* v_elite + beta .* v_feas + gamma .* v_grad + delta .* v_rand;
    mutant = popdecs + F .* v;
