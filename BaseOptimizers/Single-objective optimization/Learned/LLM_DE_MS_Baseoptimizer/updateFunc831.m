% MATLAB Code
function [offspring] = updateFunc831(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    eps = 1e-12;
    ...
    beta = 0.3 * tanh(w_c);
    gamma = 0.15 * (1 + sin(pi * w_f));
    delta = 0.05 * ones(NP, 1);
    eta = 0.05 * ones(NP, 1);
    
    % Combined mutation vector with adaptive F
    F = 0.7 + 0.2 * (1 - w_c);
    v = alpha .* v_elite + beta .* v_feas + gamma .* v_grad + ...
        delta .* v_rand + eta .* v_opp;
    mutant = popdecs + F .* v;
