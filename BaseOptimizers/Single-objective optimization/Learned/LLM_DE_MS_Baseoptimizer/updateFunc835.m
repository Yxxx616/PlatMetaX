% MATLAB Code
function [offspring] = updateFunc835(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    eps = 1e-12;
    ...
    % Adaptive parameters
    alpha = 0.8 * (1 - tanh(w_c));
    beta = 0.15 * tanh(w_c);
    gamma = 0.05 * (1 + sin(pi*w_f));
    F = 0.9 + 0.1*(1-w_c);
    
    % Mutation
    mutant = popdecs + F.*(alpha.*v_elite + beta.*v_feas + gamma.*v_grad);
