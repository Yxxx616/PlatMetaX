% MATLAB Code
function [offspring] = updateFunc845(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    eps = 1e-12;
    ...
    % Adaptive parameters with improved tuning
    F = 0.5 + 0.4 * (1 - w_c) .* (1 - w_f);
    CR = 0.8 - 0.3 * w_f;
    
    % Mutation with adjusted weights
    mutant = popdecs + F .* (v1 + 0.6 * v2 + 0.2 * v3);
