% MATLAB Code
function [offspring] = updateFunc868(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    eps = 1e-12;
    ...
    % Rank individuals based on constrained fitness
    [~, sorted_idx] = sort(popfits + 1e6*(~feasible).*cons);
    rank = zeros(NP,1);
