% MATLAB Code
function [offspring] = updateFunc880(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    
    % Find best solution (feasible first)
    feasible = cons <= 0;
    if any(feasible)
    ...
    offspring(below) = lb_rep(below) + 0.5*rand(sum(below(:)),1).*(popdecs(below) - lb_rep(below));
    offspring(above) = ub_rep(above) - 0.5*rand(sum(above(:)),1).*(ub_rep(above) - popdecs(above));
