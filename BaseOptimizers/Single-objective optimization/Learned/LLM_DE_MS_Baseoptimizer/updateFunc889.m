% MATLAB Code
function [offspring] = updateFunc889(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    
    % Enhanced feasibility-aware ranking with dynamic penalty
    alpha = 1.8; % Increased penalty factor
    feasible = cons <= 0;
    rank_scores = popfits;
    rank_scores(~feasible) = rank_scores(~feasible) + alpha * (abs(cons(~feasible)).^1.2);
    
    % Sort population by rank scores
    [~, sorted_idx] = sort(rank_scores);
    elite_size = max(4, round(NP/3)); % Increased elite pool
    K = min(6, elite_size); % More direction vectors
    ...
    p_elite = 0.8; % Higher elite probability
    beta = 0.25; % Adjusted constraint influence
    ...
    for i = 1:NP
        ...
        constraint_term = beta * sign(cons(i)) * abs(cons(i))^1.1 * r;
    ...
    fitness_ratio = (rank_scores - min(rank_scores)) ./ (max(rank_scores) - min(rank_scores) + eps);
    scale_factors = 0.3 + 0.4 * fitness_ratio;
    ...
    offspring = offspring + 0.01 * (ub - lb) .* randn(NP, D);
    offspring = max(min(offspring, ub_rep), lb_rep);
