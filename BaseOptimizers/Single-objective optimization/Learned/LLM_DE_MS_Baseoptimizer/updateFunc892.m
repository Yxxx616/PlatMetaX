% MATLAB Code
function [offspring] = updateFunc892(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    for i = 1:NP
        ...
        F_fit = 0.5 * F * (1 + (base_fit - worst_fit)/(best_fit - worst_fit + eps));
        ...
        % Composite mutation
        mutant = base + F * directional_vec + F_fit * delta_fit + constraint_term;
