% MATLAB Code
function [offspring] = updateFunc902(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    
    % 1. Calculate fitness weights (emphasizing better solutions)
    mean_fit = mean(popfits);
    std_fit = std(popfits) + eps;
    weights = exp(-5*(popfits - mean_fit)/std_fit);
    ...
    % 3. Fitness-guided direction (vectorized)
    ...
    mutants = popdecs + bsxfun(@times, F, term1 + term2) + bsxfun(@times, beta, d_r);
