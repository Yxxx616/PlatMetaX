% MATLAB Code
function [offspring] = updateFunc927(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    directional_update = ~mask & (rand(NP, D) < 0.1);
    offspring(directional_update) = popdecs(directional_update) + ...
        gamma .* (mutants(directional_update) - popdecs(directional_update));
    
    % 7. Projected boundary handling
    below = offspring < lb;
    above = offspring > ub;
    offspring(below) = (lb(below) + popdecs(below)) / 2;
    offspring(above) = (ub(above) + popdecs(above)) / 2;
