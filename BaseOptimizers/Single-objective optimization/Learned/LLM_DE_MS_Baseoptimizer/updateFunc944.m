% MATLAB Code
function [offspring] = updateFunc944(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    alpha = 0.5 * w .* (fitness_diff ./ fitness_denom);
    ...
    % 8. Boundary handling with base vector reference
    ...
    % For any remaining out-of-bounds, use base vector reference
    still_violating = offspring < lb | offspring > ub;
    rand_offsets = rand(sum(still_violating(:)), 1) .* (ub(still_violating) - lb(still_violating));
    offspring(still_violating) = x_base(still_violating) + rand_offsets;
