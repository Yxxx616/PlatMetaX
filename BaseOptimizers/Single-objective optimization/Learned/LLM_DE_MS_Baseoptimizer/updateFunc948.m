% MATLAB Code
function [offspring] = updateFunc948(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    mutants = x_base(ones(NP,1),:) + ...
              F(:, ones(1,D)) .* diff1 + ...
              0.5 * F(:, ones(1,D)) .* fitness_ratio(:, ones(1,D)) .* diff2;
    
    % 6. Adaptive crossover rate (0.1 to 0.9)
    CR = 0.1 + 0.8 * (1 - c_norm);
    ...
    % Final bounds enforcement in case reflection goes out again
