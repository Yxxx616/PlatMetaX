% MATLAB Code
function [offspring] = updateFunc949(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    % 3. Compute adaptive scaling factors (0.2 to 0.9)
    F = 0.2 + 0.7 * (1 - c_norm) .* (1 - f_norm);
    ...
    mutants = x_base(ones(NP,1),:) + ...
              F(:, ones(1,D)) .* diff1 + ...
              0.6 * F(:, ones(1,D)) .* fitness_ratio(:, ones(1,D)) .* diff2;
    
    % 6. Adaptive crossover rate (0.15 to 0.95)
    CR = 0.15 + 0.8 * (1 - c_norm);
