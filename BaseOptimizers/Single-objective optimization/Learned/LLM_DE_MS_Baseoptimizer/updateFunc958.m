% MATLAB Code
function [offspring] = updateFunc958(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    f_min = min(popfits);
    f_max = max(popfits);
    F = 0.5 * (1 + abs(cons)/c_max) .* (1 - (popfits - f_min)/(f_max - f_min + eps));
    CR = 0.9 * (1 - abs(cons)/c_max).^0.5;
    ...
    dir1 = x_best - x_worst;
    dir2 = popdecs(r(:,1),:) - popdecs(r(:,2),:) + popdecs(r(:,3),:) - popdecs(r(:,4),:);
    ...
    % Reflection based on fitness rank
    [~, rank_order] = sort(popfits + 10*max(0, cons));
    rank_coeff = (rank_order/NP).^0.5;
