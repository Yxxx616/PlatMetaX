% MATLAB Code
function [offspring] = updateFunc990(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    ...
    [~, elite_idx] = mink(norm_fits + norm_cons, 3);
    elite_pop = popdecs(elite_idx, :);
    ...
    elite_term = mean(elite_pop) - popdecs;
    
    weights = exp(-(norm_fits(r1) + norm_fits(r2) + norm_cons(r1) + norm_cons(r2))/2);
    weights = weights ./ sum(weights, 2);
    diff_terms = sum((popdecs(r1,:) - popdecs(r2,:)) .* weights(:,[1 1 1]), 2);
    
    repair_term = sign(cons) .* randn(NP, D) .* norm_cons;
