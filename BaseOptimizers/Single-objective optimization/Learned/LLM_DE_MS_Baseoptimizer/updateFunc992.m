% MATLAB Code
function [offspring] = updateFunc992(popdecs, popfits, cons)
    [NP, D] = size(popdecs);
    lb = -100 * ones(1, D);
    ub = 100 * ones(1, D);
    
    % Normalize constraints using tanh for smooth scaling
    ...
    % Combined mutation
    mutants = popdecs + F_e.*elite_term + F_d.*diff_terms + F_c.*perturbation;
