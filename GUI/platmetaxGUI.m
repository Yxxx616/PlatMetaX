...
%                 platmetaxGUI.APP([1 2],7,uiimage(obj.app.grid(2),'ImageSource',obj.icon.(['image',index]),'ImageClickedFcn',@(~,~)web(['https://bimk.github.io/Conference-Competition/?page=',index],'-browser')));
%             end
        end
    end
	methods(Access = private)
        ...
        function cb_module(obj,~,~,GUIsetting)
            name = {'platmetaxmodule_test','platmetaxmodule_app','platmetaxmodule_exp'};
            for i = 1 : length(name)
            ...
            save(fullfile(fileparts(mfilename('fullpath')),'platmetaxGUI'),'GUIsetting','-append');
        end
        %% Show the figure of about PlatMetaX
        function cb_author(obj,~,~)
            P = obj.app.figure.Position;
            C = obj.app.figure.CurrentPoint;
        end
        %% Load images after closing the platmetaxGUI
        function delete(obj)
        end
    end
    methods(Static)
        ...
        function [stateButton,label] = GenerateLabelButton(grid,values,cbFcn)
            ...
            stateButton(10) = platmetaxGUI.APP(7,2,uibutton(grid,'state','Text','constrained','FontSize',11,'FontColor',[.15 .6 .2],'BackgroundColor','w','Value',values(10),'Tooltip','The problem has constraints','ValueChangedFcn',{cbFcn,10}));
            stateButton(11) = platmetaxGUI.APP(7,3,uibutton(grid,'state','Text','expensive','FontSize',11,'FontColor',[.15 .6 .2],'BackgroundColor','w','Value',values(11),'Tooltip','The objectives are computationally time-consuming','ValueChangedFcn',{cbFcn,11}));
            stateButton(12) = platmetaxGUI.APP(8,1,uibutton(grid,'state','Text','multimodal','FontSize',11,'FontColor',[.15 .6 .2],'BackgroundColor','w','Value',values(12),'Tooltip','The objectives are multimodal','ValueChangedFcn',{cbFcn,12}));
            stateButton(13) = platmetaxGUI.APP(8,2,uibutton(grid,'state','Text','sparse','FontSize',11,'FontColor',[.15 .6 .2],'BackgroundColor','w','Value',values(13),'Tooltip','Most decision variables of the optimal solutions are zero','ValueChangedFcn',{cbFcn,13}));
            stateButton(14) = platmetaxGUI.APP(8,3,uibutton(grid,'state','Text','dynamic','FontSize',11,'FontColor',[.15 .6 .2],'BackgroundColor','w','Value',values(14),'Tooltip','The objectives vary periodically','ValueChangedFcn',{cbFcn,14}));
            stateButton(15) = platmetaxGUI.APP(9,1,uibutton(grid,'state','Text','multitask','FontSize',11,'FontColor',[.15 .6 .2],'BackgroundColor','w','Value',values(15),'Tooltip','The problem has multiple tasks to be solved simultaneously','ValueChangedFcn',{cbFcn,15}));
            stateButton(16) = platmetaxGUI.APP(9,2,uibutton(grid,'state','Text','robust','FontSize',11,'FontColor',[.15 .6 .2],'BackgroundColor','w','Value',values(16),'Tooltip','The problem has two nested objectives','ValueChangedFcn',{cbFcn,16}));
            stateButton(17) = platmetaxGUI.APP(9,3,uibutton(grid,'state','Text','learned','FontSize',11,'FontColor',[.15 .6 .2],'BackgroundColor','w','Value',values(17),'Tooltip','The optimized algorithm is learned','ValueChangedFcn',{cbFcn,17}));
        end
        ...
        function func = UpdateAlgProList(index,stateButton,varargin)
            ...
            platmetaxGUI.UpdateAlgProListYear(varargin{:});
        end
