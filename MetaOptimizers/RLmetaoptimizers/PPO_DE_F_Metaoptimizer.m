classdef PPO_DE_F_Metaoptimizer < rl.agent.AbstractOnPolicyPGAgent & ...
        rl.agent.mixin.ExperienceParallelTrainable
    ...
    methods
        function this = PPO_DE_F_Metaoptimizer(varargin)
            % Constructor
            AgentType = "PPO";
            FirstArg = varargin{1};
            ...
            elseif isa(FirstArg,'rl.util.RLDataSpec')
                % AGENT = rlPPOAgent(OINFO,AINFO)
                % AGENT = rlPPOAgent(OINFO,AINFO,INITOPTIONS)
                % AGENT = rlPPOAgent(OINFO,AINFO,AGENTOPTIONS)
                % AGENT = rlPPOAgent(OINFO,AINFO,INITOPTIONS,AGENTOPTIONS)
                [Actor, Critic, AgentOptions] = rl.agent.AbstractOnPolicyPGAgent.parseAgentInitializationInputs(AgentType,varargin{:});
            else
                error(message('rl:agent:errOnPolicyInvalidFirstArg'));
            end
            rl.util.validateAgentOptionType(AgentOptions,AgentType);
            this = this@rl.agent.AbstractOnPolicyPGAgent(Actor, Critic, AgentOptions);
    ...
    methods(Access = protected)
        ...
        function validateOptionImpl(this,NewOptions)
            rl.util.validateAgentOptionType(NewOptions,'PPO')
        ...
        % Construct a copy of the PPO agent in sim with exploration mode
        % for parallel training
        function policy = getParallelWorkerPolicyForTrainingImpl(this)
            % return a copy of the agent and make sure it is in sim with
            % exploration mode
            policy = copy(this);
            setStepMode(policy,"sim-with-exploration");
        end
        
        % Get the actor and critic learnable parameters
        function p = getParallelWorkerPolicyParametersImpl(this)
            % just return the parameters of the agent
            p = getLearnableParameters(this);
        end
        ...
        % Process the experiences on the workers before sending to the
        % learner
        function experiences = preProcessExperiencesOnWorkerImpl(this,experiences)
            % process the experiences on the workers
            experiences = preprocessExperience(this,experiences);
        end
    end
    ...
    methods (Hidden)
        function ProcessedExperience = preprocessExperience(this, Experience)
            ...
            if isa(this.ActionInfo,'rl.util.rlNumericSpec')
                ...
                OldActionProb = evaluatePolicy(this.Actor, OldActionProb, Action{1});
