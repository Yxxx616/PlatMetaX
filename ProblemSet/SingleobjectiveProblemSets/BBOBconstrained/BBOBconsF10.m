...
%--------------------------------------------------------------------------
    ...
    methods
        function Setting(obj)
            ...
            obj.feasible_direction = setFeasibleDirection(obj.evaluateGradient(obj.xinit-obj.xopt), obj.xopt,obj.D,obj.rseed);obj.fshift = generateFopt(1,1);
            obj.encoding = ones(1, obj.D);
        end
        function PopObj = CalObj(obj, X)
            Z = X - repmat(obj.xopt, size(X, 1), 1);
            weights = (1:obj.D).^2; % Ellipsoid weights
            PopObj = obj.fshift + sum(Z.^2 .* repmat(weights, size(X, 1), 1), 2);
        end
