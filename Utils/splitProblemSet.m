...
switch problemset.psName
    ...
    case 'BBOBconstrained'
        testList = [1,5,7,11,13,17];
        count1 = 1;
        count2 = 1;
        for i = 1:18
            pName = sprintf('BBOBconsF%d', i);
            if ismember(i, testList)
                testingSet{count1} = eval([pName '(pSetting{:})']);
                count1 = count1 + 1;
            else
    ...
    case 'CEC2010LLM'
        for i = 1:18
            pName = sprintf('CEC2010_F%d', i);
            trainingSet{i} = eval([pName '(pSetting{:})']);
        end
        testingSet = trainingSet;
    otherwise
        trainingSet{1} = eval([problemset.psName '(pSetting{:})']);
        testingSet{1} = eval([problemset.psName '(pSetting{:})']);
end
